...
#include "structural_analysis_element_utilities.h"
    ...
    )
{
    ...
    if (r_properties.Has( DENSITY )){
        for (IndexType i_node = 0; i_node < r_geometry.size(); ++i_node){
            density += r_properties.GetValue(DENSITY, r_geometry[i_node], rValues.GetProcessInfo());
        }
        density /= r_geometry.size();
    }
    ...
    if( r_geometry[0].SolutionStepsDataHas(VOLUME_ACCELERATION) ) {
        ...
        for (IndexType i_node = 0; i_node < r_geometry.size(); ++i_node){
            double density_node = r_properties.GetValue(DENSITY, r_geometry[i_node], rValues.GetProcessInfo());
            noalias(body_force) += N[i_node] * density_node * r_geometry[i_node].FastGetSolutionStepValue(VOLUME_ACCELERATION);
        }
    }
    return body_force;
}
    ...
    const ProcessInfo& rCurrentProcessInfo)
{
    ...
    // The default for all elements in StructuralAnalysis is
...
double GetDensityForMassMatrixComputation(const Element& rElement, const ProcessInfo& rCurrentProcessInfo)
{
    ...
    for (IndexType i_node = 0; i_node < r_geom.size(); ++i_node){
        density += r_prop.GetValue(DENSITY, r_geom[i_node], rCurrentProcessInfo);
    }
    density /= r_geom.size();
...
double CalculatePhi(ConstitutiveLaw::Parameters &rValues, const double L)
{
    ...
    const double E   = ConstitutiveLawUtilities<3>::CalculateMultiPhaseProperties(YOUNG_MODULUS, rValues);
