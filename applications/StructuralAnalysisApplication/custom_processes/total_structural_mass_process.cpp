...
double TotalStructuralMassProcess::CalculateElementMass(Element& rElement, const std::size_t DomainSize, const ProcessInfo& rCurrentProcessInfo)
{
    KRATOS_TRY
    ...
    if (local_space_dimension == 0) { // POINT MASSES
        ...
        for (std::size_t i_node = 0; i_node < number_of_nodes; ++i_node){
            double density_node = this_properties.GetValue(DENSITY, r_this_geometry[i_node], rCurrentProcessInfo);
            density_all_node += density_node;
        }
        const double density = density_all_node / number_of_nodes;
...
void TotalStructuralMassProcess::Execute()
{
    ...
    KRATOS_ERROR_IF((domain_size!=2)&&(domain_size!=3)) << "\"DOMAIN_SIZE\" for ModelPart \"" << mrThisModelPart.FullName() << "\" is " << domain_size << " but can only be 2 or 3!" << std::endl;
    ...
    for (auto& elem_i : elements_array) {
        total_mass += CalculateElementMass(elem_i, domain_size, r_process_info);
    }
