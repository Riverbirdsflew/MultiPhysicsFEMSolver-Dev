...
/** Inserts a M4dTable
*/
void ModelPart::AddM4dTable(ModelPart::IndexType M4dTableId, ModelPart::M4dTableType::Pointer pNewM4dTable)
{
    if (IsSubModelPart())
        mpParentModelPart->AddM4dTable(M4dTableId, pNewM4dTable);

    mM4dTables.insert(M4dTableId, pNewM4dTable);
}

/** Remove the M4dTable with given Id from current mesh.
*/
void ModelPart::RemoveM4dTable(ModelPart::IndexType M4dTableId)
{
    mM4dTables.erase(M4dTableId);

    for (SubModelPartIterator i_sub_model_part = SubModelPartsBegin(); i_sub_model_part != SubModelPartsEnd(); i_sub_model_part++)
        i_sub_model_part->RemoveM4dTable(M4dTableId);
}

/** Remove the M4dTable with given Id from current mesh in parents, itself and all children.
*/
void ModelPart::RemoveM4dTableFromAllLevels(ModelPart::IndexType M4dTableId)
{
    if (IsSubModelPart())
    {
        mpParentModelPart->RemoveM4dTableFromAllLevels(M4dTableId);
        return;
    }

    RemoveM4dTable(M4dTableId);
}
...
void ModelPart::PrintData(std::ostream& rOStream) const
{
    ...
    rOStream << "    Number of m4dtables : " << NumberOfM4dTables() << std::endl;
...
void ModelPart::PrintData(std::ostream& rOStream, std::string const& PrefixString) const
{
    ...
    rOStream << PrefixString << "    Number of m4dtables : " << NumberOfM4dTables() << std::endl;
