...
def CreateSolverByParameters(model, solver_settings, parallelism):
    ...
    if (parallelism == "OpenMP"):
        ...
        if (solver_type == "transient" or solver_type == "Transient"):
            ...
            if time_integration_method == "implicit":
                solver_module_name = "concentration_diffusion_transient_solver"
            elif time_integration_method == "explicit":
                solver_module_name = "concentration_diffusion_explicit_solver"
            elif time_integration_method == "semi_implicit":
                solver_module_name = "concentration_diffusion_semi_eulerian_solver"
            else:
        ...
        # Steady solver
            ...
            solver_module_name = "concentration_diffusion_stationary_solver"
        # Steady Shifted Boundary Method (SBM) solver
        elif solver_type == "stationary_shifted_boundary":
            solver_module_name = "concentration_diffusion_stationary_shifted_boundary_solver"
        # Steady embedded (CutFEM) solver
        elif solver_type == "stationary_embedded":
            solver_module_name = "concentration_diffusion_stationary_embedded_solver"
        # Auxiliary solver to generate the stationary system matrix
        elif (solver_type == "stationary_matrix"):
            solver_module_name = "concentration_diffusion_stationary_matrix_solver"
        ...
        # Wrong solver check
            ...
            err_msg += "Available options are: \"transient\", \"stationary\", \"thermal_diffusion_coupled\", \"conjugate_heat_transfer\" and \"adjoint_stationary\""
            raise Exception(err_msg)
    ...
    module_full = 'KratosMultiphysics.ConcentrationDiffusionApplication.' + solver_module_name
