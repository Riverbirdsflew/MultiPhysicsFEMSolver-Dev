...
class GenericConstitutiveLawIntegratorPlasticity
{
  public:
    ...
    enum class HardeningCurveType
    {
        ...
        CurveDefinedByPointsNoSoftening = 7,
        CurveDefinedByTable = 8,
        CurveDefinedByM4dTable = 9
    };
        ...
        )
    {
        ...
        double J2, tensile_indicator_factor, compression_indicator_factor, slope, hardening_parameter, equivalent_plastic_strain, delta_equivalent_plastic_strain;
        ...
        AdvancedConstitutiveLawSmallStrainUtilities<VoigtSize>::CalculateJ2Invariant(rPredictiveStressVector, I1, deviator, J2);
        ...
        CalculateDeltaEquivalentPlasticStrain(rPredictiveStressVector, rUniaxialStress, rPlasticStrain, equivalent_plastic_strain, rPlasticStrainIncrement, delta_equivalent_plastic_strain);
        CalculateEquivalentStressThreshold(rPlasticDissipation, tensile_indicator_factor,compression_indicator_factor, rThreshold, slope, rValues, equivalent_plastic_strain, delta_equivalent_plastic_strain, CharacteristicLength);
        ...
        )
    {
        ...
        AdvancedConstitutiveLawSmallStrainUtilities<VoigtSize>::CalculatePrincipalStresses(principal_stresses, rPredictiveStressVector);
        ...
        )
    {
        ...
        const double young_modulus = ConstitutiveLawUtilities<VoigtSize>::CalculateMultiPhaseProperties(YOUNG_MODULUS, rValues);
        //std::cout << young_modulus << std::endl;
        //const double young_modulus = r_material_properties.GetValue(YOUNG_MODULUS, r_geom, r_N, r_process_info);//修改为从accessor取值
        ...
        KRATOS_ERROR_IF(CharacteristicLength > hlim) << "The Fracture Energy is to low: " << characteristic_fracture_energy_compression << " hlim " << hlim << " ChLength "<< CharacteristicLength << " young_modulus " << young_modulus << " fracture_energy_compression " << fracture_energy_compression << "voigsize" << VoigtSize << std::endl;
     ...
     * @param PlasticDissipation The internal variable of energy dissipation due to plasticity  //塑性耗散能内部变量，表示由于材料进入塑性状态导致的能量耗散
     * @param TensileIndicatorFactor The tensile indicator  //拉伸
     * @param CompressionIndicatorFactor The compressive indicator  //压缩
     * @param rEquivalentStressThreshold The maximum uniaxial stress of the linear behaviour  //等效应力阈值，即材料在弹性状态下可以承受的最大单轴应力
     * @param rSlope The slope of the PlasticDiss-Threshold curve  //塑性耗散-应力阈值曲线的斜率
     * @param rValues Parameters of the constitutive law  //材料的本构参数，包括材料的弹性模量、屈服应力等信息
     * @param EquivalentPlasticStrain The equivalent plastic strain  //等效塑性应变
     * @param CharacteristicLength Characteristic length of the finite element  //单元的特征长度
     */
    static void CalculateEquivalentStressThreshold(
        ...
        const double DeltaEquivalentPlasticStrain,
        const double CharacteristicLength
        )
    {
        const Properties& r_material_properties = rValues.GetMaterialProperties();
        ...
        for (IndexType i = 0; i < 2; ++i) { // i:0 Tension ; i:1 compression
            switch (static_cast<HardeningCurveType>(curve_type))
            {
                ...
                case HardeningCurveType::CurveDefinedByPointsNoSoftening:
                    CalculateEquivalentStressThresholdHardeningCurveDefinedByPointsNoSoftening(
                        PlasticDissipation, TensileIndicatorFactor,
                        CompressionIndicatorFactor, eq_thresholds[i], slopes[i],
                        rValues, EquivalentPlasticStrain, CharacteristicLength);
                    break;

                case HardeningCurveType::CurveDefinedByTable:
                    CalculateEquivalentStressThresholdHardeningCurveDefinedByTable(
                        PlasticDissipation, TensileIndicatorFactor,
                        CompressionIndicatorFactor, eq_thresholds[i], slopes[i],
                        rValues, CharacteristicLength);
                    break;

                case HardeningCurveType::CurveDefinedByM4dTable:
                    CalculateEquivalentStressThresholdHardeningCurveDefinedByM4dTable(
                        PlasticDissipation, TensileIndicatorFactor,
                        CompressionIndicatorFactor, eq_thresholds[i], slopes[i],
                        rValues, EquivalentPlasticStrain, DeltaEquivalentPlasticStrain, CharacteristicLength);
                    break;
        ...
        )
    {
        ...
        const double young_modulus = ConstitutiveLawUtilities<VoigtSize>::CalculateMultiPhaseProperties(YOUNG_MODULUS, rValues);//multiphase, through accessor
        //const double young_modulus = r_material_properties.GetValue(YOUNG_MODULUS, r_geom, r_N, r_process_info);//修改为从accessor取值
        //const double young_modulus = r_material_properties[YOUNG_MODULUS];
        const bool has_symmetric_yield_stress = r_material_properties.Has(YIELD_STRESS);
        const double yield_compression = has_symmetric_yield_stress ? r_material_properties[YIELD_STRESS] : r_material_properties[YIELD_STRESS_COMPRESSION];
    ...
    static void CalculateEquivalentStressThresholdHardeningCurveDefinedByPointsNoSoftening(
        const double PlasticDissipation,
        const double TensileIndicatorFactor,
        const double CompressionIndicatorFactor,
        double& rEquivalentStressThreshold,
        ...
        )
    {
        ...
        const SizeType points_hardening_curve = equivalent_stress_vector.size();
        //double PlasticStrain = r_material_properties.GetValue(EQUIVALENT_PLASTIC_STRAIN);
        const double eq_PlasticStrain = EquivalentPlasticStrain;
        
        //把一些结果打印到文本文件中
        const auto& r_geom = rValues.GetElementGeometry();
        const ProcessInfo& r_process_info = rValues.GetProcessInfo();
        const bool first_computation = (r_process_info[NL_ITERATION_NUMBER] == 1 && r_process_info[STEP] == 1) ? true : false;
        if(first_computation){
            std::ofstream output_file("eq_plastic_strain.txt", std::ios::trunc);
            output_file.close();
        }
        if (r_geom[0].Id() == 1){
            std::ofstream output_file;
            output_file.open("eq_plastic_strain.txt", std::ios::app);
            output_file<<"### STEP"<<r_process_info[STEP]<<"### ITERATION" << r_process_info[NL_ITERATION_NUMBER] <<"###PlasticStrain: "<<eq_PlasticStrain<<std::endl;
            output_file.close();
        }             
        ...
        //     rEquivalentStressThreshold = equivalent_stress_vector[0]; // 初始屈服应力
        //     rSlope = 0.0;  // 弹性阶段不需要塑性硬化
        // } 
        if (eq_PlasticStrain <= plastic_strain_vector[0]) {
            rEquivalentStressThreshold = equivalent_stress_vector[0];
            rSlope = (equivalent_stress_vector[1] - equivalent_stress_vector[0]) / (plastic_strain_vector[1] - plastic_strain_vector[0]);
            //rSlope = 0.0;
        }else if(eq_PlasticStrain >= plastic_strain_vector[points_hardening_curve - 1]){
            rEquivalentStressThreshold = equivalent_stress_vector[points_hardening_curve - 1];
            rSlope = 0.0;
        }
        else {
            for(IndexType i = 1; i < points_hardening_curve; ++i){
                if(eq_PlasticStrain < plastic_strain_vector[i]){
                    const double slope = (equivalent_stress_vector[i] - equivalent_stress_vector[i - 1]) / (plastic_strain_vector[i] - plastic_strain_vector[i - 1]);
                    rEquivalentStressThreshold = equivalent_stress_vector[i - 1] + slope * (eq_PlasticStrain - plastic_strain_vector[i - 1]);
                    rSlope = slope;
                    break;
                }
            }
    ...
    //把应力应变曲线读取通过向量改为通过表格
    static void CalculateEquivalentStressThresholdHardeningCurveDefinedByTable(
        const double PlasticDissipation,
        const double TensileIndicatorFactor,
        const double CompressionIndicatorFactor,
        double& rEquivalentStressThreshold,
        ...
        )
    {
        ...
        const bool has_plastic_stress_strain_table = r_material_properties.HasTable(EQUIVALENT_STRESS_VECTOR_PLASTICITY_POINT, PLASTIC_STRAIN_VECTOR_PLASTICITY_POINT);
        const bool has_plastic_stress_strain_m4dtable = r_material_properties.HasM4dTable(1,EQUIVALENT_STRESS_VECTOR_PLASTICITY_POINT,EQUIVALENT_STRESS_VECTOR_PLASTICITY_POINT,EQUIVALENT_STRESS_VECTOR_PLASTICITY_POINT,EQUIVALENT_STRESS_VECTOR_PLASTICITY_POINT, PLASTIC_STRAIN_VECTOR_PLASTICITY_POINT);
        //const double young_modulus = ConstitutiveLawUtilities<VoigtSize>::CalculateMultiPhaseProperties(YOUNG_MODULUS, rValues);//multiphase, through accessor

        if(has_plastic_stress_strain_table){
            Table<double> plastic_strain_stress_table = r_material_properties.GetTable(EQUIVALENT_STRESS_VECTOR_PLASTICITY_POINT, PLASTIC_STRAIN_VECTOR_PLASTICITY_POINT);
            ...
            const SizeType points_hardening_curve = plastic_strain_stress_table.Data().size();
            //从初始积分到给定应力向量的最后
            Gt1 = plastic_strain_stress_table.GetIntegration(plastic_strain_stress_table.Data()[0].first, plastic_strain_stress_table.Data()[points_hardening_curve-1].first);
            
            //~ for (std::size_t i = 1; i < points_hardening_curve; ++i) {
                //~ Gt1 += 0.5 * (equivalent_stress_vector(i - 1) + equivalent_stress_vector(i)) * (plastic_strain_vector(i) - plastic_strain_vector(i - 1));
            //~ }
            const double Gt2 = volumetric_fracture_energy - Gt1;
            ...
            if (PlasticDissipation < segment_threshold) {
                ...
                while (PlasticDissipation >= gf_point_region / volumetric_fracture_energy) {
                    i += 1;
                    plastic_dissipation_previous_point = gf_point_region / volumetric_fracture_energy;//保存当前PlasticDissipation所落区间的起始点处的塑性耗散量
                    gf_point_region = plastic_strain_stress_table.GetIntegration(plastic_strain_stress_table.Data()[0].first, plastic_strain_stress_table.Data()[i].first);
                    
                    //gf_point_region += 0.5 * (equivalent_stress_vector(i - 1) + equivalent_stress_vector(i)) * (plastic_strain_vector(i) - plastic_strain_vector(i - 1));//累加当前点和上一个点的平均应力乘以应变增量
                }
                const double plastic_dissipation_next_point = gf_point_region / volumetric_fracture_energy;//保存当前PlasticDissipation所落区间的终止点处的塑性耗散量
                //循环结束后，i值确定了塑性耗散能所在的两个点之间的区间

                // Stress is computed using an equivalent equation to the one used for the linear softening curve, i.e. f(S) = a * (1.0 - b * kp)
                const double b = (std::pow(plastic_strain_stress_table.Data()[i].second[0], 2.0) - std::pow(plastic_strain_stress_table.Data()[i-1].second[0], 2.0)) / (plastic_dissipation_previous_point * std::pow(plastic_strain_stress_table.Data()[i].second[0], 2.0) - plastic_dissipation_next_point * std::pow(plastic_strain_stress_table.Data()[i-1].second[0], 2.0));
                
                //const double b = (std::pow(equivalent_stress_vector(i), 2.0) - std::pow(equivalent_stress_vector(i - 1), 2.0)) / (plastic_dissipation_previous_point * std::pow(equivalent_stress_vector(i), 2.0) - plastic_dissipation_next_point * std::pow(equivalent_stress_vector(i - 1), 2.0));//计算硬化曲线在当前区间的非线性变化速率，通过两个相邻点的应力平方差和塑性耗散差定义
                const double a = plastic_strain_stress_table.Data()[i-1].second[0] / std::sqrt(1.0 - b * plastic_dissipation_previous_point);
                
                //const double a = equivalent_stress_vector(i - 1) / std::sqrt(1.0 - b * plastic_dissipation_previous_point);//线性插值曲线在当前区间的起点幅度，即以插值方式定义的屈服应力
                rEquivalentStressThreshold = a * std::sqrt(1.0 - b * PlasticDissipation);
                rSlope = - 0.5 * std::pow(a, 2.0) * b / rEquivalentStressThreshold;
                ...
                const bool total_or_plastic_strain_space = has_total_or_plastic_strain_space ? r_material_properties[TOTAL_OR_PLASTIC_STRAIN_SPACE] : false; // Default value = plastic strain space
                if (total_or_plastic_strain_space) { // Curve built in the total strain space
                    const double b = (std::pow(plastic_strain_stress_table.Data()[points_hardening_curve - 1].second[0], 2.0) - std::pow(plastic_strain_stress_table.Data()[points_hardening_curve-2].second[0], 2.0)) / (plastic_dissipation_previous_point * std::pow(plastic_strain_stress_table.Data()[points_hardening_curve - 1].second[0], 2.0) - plastic_dissipation_next_point * std::pow(plastic_strain_stress_table.Data()[points_hardening_curve - 2].second[0], 2.0));
                

                    const double a = plastic_strain_stress_table.Data()[points_hardening_curve - 1].second[0] / std::sqrt(1.0 - b * plastic_dissipation_previous_point);
        ...
        else if(has_plastic_stress_strain_m4dtable) {
            //m4table给出的塑性应力应变曲线
            M4dTable plastic_strain_stress_table = r_material_properties.GetM4dTable(2,EQUIVALENT_STRESS_VECTOR_PLASTICITY_POINT,EQUIVALENT_STRESS_VECTOR_PLASTICITY_POINT,EQUIVALENT_STRESS_VECTOR_PLASTICITY_POINT,EQUIVALENT_STRESS_VECTOR_PLASTICITY_POINT, PLASTIC_STRAIN_VECTOR_PLASTICITY_POINT);
            ...
            const SizeType points_hardening_curve = plastic_strain_stress_table.Data().size();
            //从初始积分到给定应力向量的最后
            Gt1 = plastic_strain_stress_table.GetIntegrationValue({plastic_strain_stress_table.Data()[0],0.0,0.0,0.0}, {plastic_strain_stress_table.Data()[points_hardening_curve-1],0.0,0.0,0.0});
            ...
            if (PlasticDissipation < segment_threshold) {
                ...
                while (PlasticDissipation >= gf_point_region / volumetric_fracture_energy) {
                    i += 1;
                    plastic_dissipation_previous_point = gf_point_region / volumetric_fracture_energy;//保存当前PlasticDissipation所落区间的起始点处的塑性耗散量
                    gf_point_region = plastic_strain_stress_table.GetIntegrationValue({plastic_strain_stress_table.Data()[0],0.0,0.0,0.0}, {plastic_strain_stress_table.Data()[i],0.0,0.0,0.0});
                ...
                // Stress is computed using an equivalent equation to the one used for the linear softening curve, i.e. f(S) = a * (1.0 - b * kp)
                const double b = (std::pow(plastic_strain_stress_table.Data()[i], 2.0) - std::pow(plastic_strain_stress_table.Data()[i-1], 2.0)) / (plastic_dissipation_previous_point * std::pow(plastic_strain_stress_table.Data()[i], 2.0) - plastic_dissipation_next_point * std::pow(plastic_strain_stress_table.Data()[i-1], 2.0));
                
                //const double b = (std::pow(equivalent_stress_vector(i), 2.0) - std::pow(equivalent_stress_vector(i - 1), 2.0)) / (plastic_dissipation_previous_point * std::pow(equivalent_stress_vector(i), 2.0) - plastic_dissipation_next_point * std::pow(equivalent_stress_vector(i - 1), 2.0));//计算硬化曲线在当前区间的非线性变化速率，通过两个相邻点的应力平方差和塑性耗散差定义
                const double a = plastic_strain_stress_table.Data()[i-1] / std::sqrt(1.0 - b * plastic_dissipation_previous_point);
                ...
                const bool total_or_plastic_strain_space = has_total_or_plastic_strain_space ? r_material_properties[TOTAL_OR_PLASTIC_STRAIN_SPACE] : false; // Default value = plastic strain space
                if (total_or_plastic_strain_space) { // Curve built in the total strain space
                    const double b = (std::pow(plastic_strain_stress_table.Data()[points_hardening_curve - 1], 2.0) - std::pow(plastic_strain_stress_table.Data()[points_hardening_curve-2], 2.0)) / (plastic_dissipation_previous_point * std::pow(plastic_strain_stress_table.Data()[points_hardening_curve - 1], 2.0) - plastic_dissipation_next_point * std::pow(plastic_strain_stress_table.Data()[points_hardening_curve - 2], 2.0));
                

                    const double a = plastic_strain_stress_table.Data()[points_hardening_curve - 1] / std::sqrt(1.0 - b * plastic_dissipation_previous_point);
        ...
        if (PlasticDissipation < segment_threshold) {
            ...
            if (total_or_plastic_strain_space) { // Curve built in the total strain space
                ...
                rEquivalentStressThreshold = a + std::sqrt(std::pow(a, 2.0) + 2.0 * equivalent_stress_vector(0) * volumetric_fracture_energy * (1.0 - PlasticDissipation) / yield_strain);
                rSlope = - equivalent_stress_vector(0) * volumetric_fracture_energy / (yield_strain * std::sqrt(std::pow(a, 2.0) + 2.0 * equivalent_stress_vector(0) * volumetric_fracture_energy * (1.0 - PlasticDissipation) / yield_strain));
                //const double b = (std::pow(equivalent_stress_vector(points_hardening_curve - 1), 2.0) - std::pow(equivalent_stress_vector(points_hardening_curve - 1), 2.0)) / (plastic_dissipation_previous_point * std::pow(equivalent_stress_vector(i), 2.0) - plastic_dissipation_next_point * std::pow(equivalent_stress_vector(i - 1), 2.0));//计算硬化曲线在当前区间的非线性变化速率，通过两个相邻点的应力平方差和塑性耗散差定义
            //const double a = equivalent_stress_vector(i - 1) / std::sqrt(1.0 - b * plastic_dissipation_previous_point);
            } else { // Curve built in the plastic strain space
                const double a = equivalent_stress_vector(points_hardening_curve - 1) / (1.0 - segment_threshold);
                rEquivalentStressThreshold =  a * (1.0 - PlasticDissipation);
    ...
    /**
     * @brief This method computes the uniaxial threshold using a m4d table:
     *          - point defined curve with linear interpolation
     * @param PlasticDissipation The internal variable of energy dissipation due to plasticity
     * @param TensileIndicatorFactor The tensile indicator
     * @param CompressionIndicatorFactor The compressive indicator
     * @param rEquivalentStressThreshold The maximum uniaxial stress of the linear behaviour
     ...
     * @param CharacteristicLength Characteristic length of the finite element
     */
    static void CalculateEquivalentStressThresholdHardeningCurveDefinedByM4dTable(
        const double PlasticDissipation,
        const double TensileIndicatorFactor,
        const double CompressionIndicatorFactor,
        double& rEquivalentStressThreshold,
    ...
    {
        const Properties& r_material_properties = rValues.GetMaterialProperties();
        M4dTable plastic_strain_stress_table = r_material_properties.GetM4dTable(2,TEMPERATURE,
                                                                                   PLASTIC_STRAIN_VECTOR_PLASTICITY_POINT,
                                                                                   PLASTIC_STRAIN_VECTOR_PLASTICITY_POINT,
                                                                                   PLASTIC_STRAIN_VECTOR_PLASTICITY_POINT, 
                                                                                   EQUIVALENT_STRESS_VECTOR_PLASTICITY_POINT);
        KRATOS_ERROR_IF(!(plastic_strain_stress_table.Size(0)) && (plastic_strain_stress_table.Size(1))) << "No M4dTable for EqStress - EqPlasticStrain and temperature " << std::endl;
        
        //double PlasticStrain = r_material_properties.GetValue(EQUIVALENT_PLASTIC_STRAIN);
        const double eq_plastic_strain = EquivalentPlasticStrain;
        const double current_temperature = AdvancedConstitutiveLawSmallStrainUtilities<VoigtSize>::CalculateInGaussPoint(TEMPERATURE, rValues, 0);             
        
        double equivalent_stress_threshold = 0.0;
        double slope = 0.0;

        double eq_plastic_strain_last = eq_plastic_strain - DeltaEquivalentPlasticStrain;
        
        equivalent_stress_threshold = plastic_strain_stress_table.GetInterpolateValue({current_temperature,eq_plastic_strain,eq_plastic_strain,eq_plastic_strain});
        slope = (plastic_strain_stress_table.GetDerivativeValue(1,{current_temperature,eq_plastic_strain,eq_plastic_strain,eq_plastic_strain}) + plastic_strain_stress_table.GetDerivativeValue(1,{current_temperature,eq_plastic_strain_last,eq_plastic_strain_last,eq_plastic_strain_last})) / 2.0;

        rEquivalentStressThreshold = equivalent_stress_threshold;
        rSlope = slope;
        ...
        if (r_geom[0].Id() == 1){
            std::ofstream output_file;
            output_file.open("eq_plastic_strain.txt", std::ios::app);
            output_file<<"### STEP"<<r_process_info[STEP]<<"### ITERATION" << r_process_info[NL_ITERATION_NUMBER] <<"###PlasticStrain: "<<eq_plastic_strain<<std::endl;
            output_file<<"### Temperature: "<<current_temperature<<std::endl;
            output_file<<"### EquivalentStressThreshold: "<<rEquivalentStressThreshold<<std::endl;
            output_file<<"### DeltaEquivalentPlasticStrain: "<<DeltaEquivalentPlasticStrain<<std::endl;
            output_file<<"### Slope: "<<rSlope<<std::endl;
            output_file.close();
        }
    }
    ...
    static void CalculateDeltaEquivalentPlasticStrain(
        const Vector& rStressVector,
        const double UniaxialStress,
        const Vector& rPlasticStrain,
        double& rEquivalentPlasticStrain,
        array_1d<double, VoigtSize>& rPlasticStrainIncrement,
        double& rDeltaEquivalentPlasticStrain
        )
    {
        double scalar_product_delta = 0.0;
        for (IndexType i = 0; i < rPlasticStrain.size(); ++i) {
            scalar_product_delta += rStressVector[i] * rPlasticStrainIncrement[i];
        }

        /*Since this law is used for Von Mises and Tresca, no
        scaling is necessary, even though the needed params are available*/
        rDeltaEquivalentPlasticStrain = scalar_product_delta / UniaxialStress;
    }
