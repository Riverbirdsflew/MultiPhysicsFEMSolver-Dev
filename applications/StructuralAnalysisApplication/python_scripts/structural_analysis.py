...
from KratosMultiphysics.StructuralAnalysisApplication import python_solvers_wrapper_structural as structural_solvers

# Importing the base class
from KratosMultiphysics.analysis_stage import AnalysisStage

class StructuralAnalysis(AnalysisStage):
    """
    This class is the main-script of the StructuralAnalysisApplication put in a class
    ...
    def __init__(self, model, project_parameters):
        ...
        if solver_settings.Has("domain_size") and project_parameters["problem_data"].Has("domain_size"):
            raise Exception("StructuralAnalysis: " + '"domain_size" defined both in "problem_data" and "solver_settings"!')

        if solver_settings.Has("model_part_name") and project_parameters["problem_data"].Has("model_part_name"):
            raise Exception("StructuralAnalysis: " + '"model_part_name" defined both in problem_data" and "solver_settings"!')

        if solver_settings.Has("time_stepping") and project_parameters["problem_data"].Has("time_Step"):
            raise Exception("StructuralAnalysis: " + '"time_stepping" defined both in "problem_data" and "solver_settings"!')

        if not solver_settings.Has("time_stepping"):
            raise Exception("StructuralAnalysis: Using the old way to pass the time_step, this was removed!")

        if not solver_settings.Has("domain_size"):
            raise Exception("StructuralAnalysis: Using the old way to pass the domain_size, this was removed!")
    ...
    def OutputSolutionStep(self):
        ...
        if self.contact_problem:
            ...
            if is_output_step:
                ...
                KratosMultiphysics.Logger.PrintWarning("StructuralAnalysis", "STEP: ", self._GetSolver().GetComputingModelPart().ProcessInfo[KratosMultiphysics.STEP])
                KratosMultiphysics.Logger.PrintWarning("StructuralAnalysis", "TIME: ", self.time)
            ...
            if len(list_of_processes) == 0: # Processes are given in the old format (or no processes are specified)
                for process_name in processes_block_names:
                    if self.project_parameters.Has(process_name):
                        ...
                        raise Exception("StructuralAnalysis: " + info_msg)
