...
import KratosMultiphysics.ThermalConductionApplication as KratosThermCond

def Factory(settings, Model):
    if(type(settings) != KratosMultiphysics.Parameters):
        raise Exception("expected input shall be a Parameters object, encapsulating a json string")
    return ApplyTableThermalFaceProcess(Model, settings["Parameters"])


class ApplyTableThermalFaceProcess(KratosMultiphysics.Process):
    def __init__(self, Model, settings):
        ...
        default_settings = KratosMultiphysics.Parameters(r'''{
            "model_part_name": "",
            "table_time_ambient_temperature": false,
            "table_time_convection_coefficient": false,
        ...
        thermal_interface_prop = model_part.GetProperties()[1]
        ...
        if settings["table_time_ambient_temperature"].GetBool():
            thermal_interface_prop.SetValue(KratosMultiphysics.AMBIENT_TEMPERATURE, 0.0)
        else:
            thermal_interface_prop.SetValue(KratosMultiphysics.AMBIENT_TEMPERATURE, ambient_temperature)
        if settings["table_time_convection_coefficient"].GetBool():
