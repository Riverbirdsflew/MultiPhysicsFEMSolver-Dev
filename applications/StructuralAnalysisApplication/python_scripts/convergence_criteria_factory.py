...
class convergence_criterion:
    def __init__(self, convergence_criterion_parameters):
        ...
        if(echo_level >= 1):
            KratosMultiphysics.Logger.PrintInfo("::[Structure Solver]:: ", "CONVERGENCE CRITERION : " +
        ...
        if(rotation_dofs):
            if(convergence_crit == "displacement_criterion"):
                ...
                self.mechanical_convergence_criterion = StructuralAnalysisApplication.ResidualDisplacementAndOtherDoFCriteria(R_RT, R_AT)
                self.mechanical_convergence_criterion.SetEchoLevel(echo_level)
                ...
                Residual = StructuralAnalysisApplication.ResidualDisplacementAndOtherDoFCriteria(R_RT, R_AT)
        ...
        # Convergence criteria if there are volumetric strain DOFs in the problem
            ...
            if(convergence_crit == "displacement_criterion"):
                ...
                self.mechanical_convergence_criterion = StructuralAnalysisApplication.ResidualDisplacementAndOtherDoFCriteria(R_RT, R_AT, other_dof_name)
                self.mechanical_convergence_criterion.SetEchoLevel(echo_level)
                ...
                Displacement = KratosMultiphysics.MixedGenericCriteria(
                    ...
                    (KratosMultiphysics.VOLUMETRIC_STRAIN, D_RT, D_AT)])
                Displacement.SetEchoLevel(echo_level)
                Residual = StructuralAnalysisApplication.ResidualDisplacementAndOtherDoFCriteria(R_RT, R_AT, other_dof_name)
                Residual.SetEchoLevel(echo_level)
                self.mechanical_convergence_criterion = KratosMultiphysics.AndCriteria(Residual, Displacement)
