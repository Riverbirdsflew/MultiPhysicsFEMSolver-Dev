...
def CreateSolverByParameters(model, solver_settings, parallelism):
    ...
    if parallelism == "OpenMP":
        if solver_type == "dynamic" or solver_type == "Dynamic":
            if time_integration_method == "implicit":
                solver_module_name = "structural_analysis_implicit_dynamic_solver"
            elif time_integration_method == "explicit":
                solver_module_name = "structural_analysis_explicit_dynamic_solver"
            else:
            ...
            solver_module_name = "structural_analysis_static_solver"

        elif solver_type == "static_shifted_boundary":
            solver_module_name = "structural_analysis_static_shifted_boundary_solver"

        elif solver_type == "eigen_value":
            solver_module_name = "structural_analysis_eigensolver"

        elif solver_type == "harmonic_analysis":
            solver_module_name = "structural_analysis_harmonic_analysis_solver"

        elif solver_type == "formfinding":
            solver_module_name = "structural_analysis_formfinding_solver"

        elif solver_type == "adjoint_static":
            solver_module_name = "structural_analysis_adjoint_static_solver"

        elif solver_type == "prebuckling":
            solver_module_name = "structural_analysis_prebuckling_solver"
    ...
    # Solvers for MPI parallelism
        ...
        if solver_type == "dynamic" or solver_type == "Dynamic":
            if time_integration_method == "implicit":
                solver_module_name = "trilinos_structural_analysis_implicit_dynamic_solver"
            else:
                err_msg =  "The requested time integration method \"" + time_integration_method + "\" is not in the python solvers wrapper\n"
            ...
            solver_module_name = "trilinos_structural_analysis_static_solver"
    ...
    if try_import_custom_solver:
        KratosMultiphysics.Logger.PrintInfo("StructuralSolversWrapper", 'Selected "solver_type" "{0}" not available in the python solvers wrapper, attempting to import custom solver from module "{0}"'.format(solver_type))
        try:
            solver = import_module(solver_type).CreateSolver(model, solver_settings)
            KratosMultiphysics.Logger.PrintInfo("StructuralSolversWrapper", 'Using custom solver "{}", defined in module "{}"'.format(solver.__class__.__name__, solver.__class__.__module__))
            return solver
        except:
    ...
    if solver_settings.Has("contact_settings"):  # This is a contact problem
        kratos_module = "KratosMultiphysics.ContactStructuralAnalysisApplication"
        ...
        kratos_module = "KratosMultiphysics.StructuralAnalysisApplication"
