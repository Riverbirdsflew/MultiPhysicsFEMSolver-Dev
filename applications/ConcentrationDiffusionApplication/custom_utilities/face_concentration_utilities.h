...
#if !defined(KRATOS_FACE_CONCENTRATION_UTILITIES_INCLUDED )
#define  KRATOS_FACE_CONCENTRATION_UTILITIES_INCLUDED
...
namespace Kratos
{
class FaceConcentrationUtilities
{
public:

    //**********************************************************************************************
    //**********************************************************************************************
    void ApplyFaceConcentration(
        ModelPart::ConditionsContainerType& conditions ,
        double face_concentration_source
    )
    {
        KRATOS_TRY
        for(ModelPart::ConditionsContainerType::iterator iii = conditions.begin(); iii != conditions.end(); iii++)  //遍历条件
        {
            Geometry< Node >& geom = iii->GetGeometry();

            for(unsigned int k = 0; k<geom.size(); k++)  //遍历节点
                geom[k].FastGetSolutionStepValue(FACE_CONCENTRATION_FLUX) = face_concentration_source;
    ...
    void ConditionModelPart(ModelPart& concentration_model_part, ModelPart& full_model_part, const int TDim)
    {
        KRATOS_TRY;


        concentration_model_part.Conditions().clear();
        int nd=TDim;
            ...
            if(nd==2)
            {
                ...
                else
                {
                    ...
                    if(n_int==2)
                    {
                        ...
                        int id = (im->Id()-1)*3;
                        Condition::Pointer p_cond = Kratos::make_intrusive<ConcentrationFace>(id, cond, properties);
                        concentration_model_part.Conditions().push_back(p_cond);
...
#endif // KRATOS_FACE_CONCENTRATION_UTILITIES_INCLUDED  defined
