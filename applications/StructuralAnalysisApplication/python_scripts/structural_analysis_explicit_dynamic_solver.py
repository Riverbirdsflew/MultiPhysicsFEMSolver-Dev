...
def CreateSolver(model, custom_settings):
    return ExplicitStructuralSolver(model, custom_settings)

class ExplicitStructuralSolver(StructuralSolver):
    ...
    def __init__(self, model, custom_settings):
        ...
        KratosMultiphysics.Logger.PrintInfo("::[ExplicitStructuralSolver]:: Construction finished")
    ...
    def AddVariables(self):
        ...
        if scheme_type == "central_differences":
            self.main_model_part.AddNodalSolutionStepVariable(StructuralAnalysisApplication.MIDDLE_VELOCITY)
            if self.settings["rotation_dofs"].GetBool():
                self.main_model_part.AddNodalSolutionStepVariable(StructuralAnalysisApplication.MIDDLE_ANGULAR_VELOCITY)
        if scheme_type == "multi_stage":
            self.main_model_part.AddNodalSolutionStepVariable(StructuralAnalysisApplication.FRACTIONAL_ACCELERATION)
            if self.settings["rotation_dofs"].GetBool():
                self.main_model_part.AddNodalSolutionStepVariable(StructuralAnalysisApplication.FRACTIONAL_ANGULAR_ACCELERATION)
        ...
        if self.settings["rotation_dofs"].GetBool():
            self.main_model_part.AddNodalSolutionStepVariable(StructuralAnalysisApplication.NODAL_INERTIA)
            self.main_model_part.AddNodalSolutionStepVariable(KratosMultiphysics.MOMENT_RESIDUAL)

        KratosMultiphysics.Logger.PrintInfo("::[ExplicitStructuralSolver]:: Variables ADDED")

    def AddDofs(self):
        super().AddDofs()
        self._add_dynamic_dofs()
        KratosMultiphysics.Logger.PrintInfo("::[ExplicitStructuralSolver]:: DOF's ADDED")
    ...
    def ComputeDeltaTime(self):
        if self.settings["time_step_prediction_level"].GetInt() > 1:
            if self.delta_time_refresh_counter >= self.settings["delta_time_refresh"].GetInt():
                self.delta_time = StructuralAnalysisApplication.CalculateDeltaTime(self.GetComputingModelPart(), self.delta_time_settings)
    ...
    def _CreateScheme(self):
        ...
        process_info[StructuralAnalysisApplication.RAYLEIGH_ALPHA] = self.settings["rayleigh_alpha"].GetDouble()
        process_info[StructuralAnalysisApplication.RAYLEIGH_BETA] = self.settings["rayleigh_beta"].GetDouble()

        # Setting the time integration schemes
        if scheme_type == "central_differences":
            mechanical_scheme = StructuralAnalysisApplication.ExplicitCentralDifferencesScheme(self.settings["max_delta_time"].GetDouble(),
            ...
            mechanical_scheme = StructuralAnalysisApplication.ExplicitMultiStageKimScheme(self.settings["fraction_delta_time"].GetDouble())
        else:
    ...
    def _CreateSolutionStrategy(self):
        ...
        mechanical_solution_strategy = StructuralAnalysisApplication.MechanicalExplicitStrategy(computing_model_part,
                                            mechanical_scheme,
