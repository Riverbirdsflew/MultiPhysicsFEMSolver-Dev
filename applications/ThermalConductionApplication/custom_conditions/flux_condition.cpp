    ...
    const ProcessInfo& rCurrentProcessInfo)
{
    ...
    FluxConditionInternals::IntegrationData<TNodeNumber> Data(this->GetGeometry(), FACE_HEAT_FLUX);
    ...
    const ProcessInfo& rCurrentProcessInfo) const
{
    ...
    {
        rResult[i] = rGeometry[i].GetDof(TEMPERATURE).EquationId();
    }
    ...
    const ProcessInfo& rCurrentProcessInfo) const
{
    ...
    for (unsigned int i = 0; i < TNodeNumber; i++)
    {
        rConditionalDofList[i] = rGeometry[i].pGetDof(TEMPERATURE);
    }
