...
import KratosMultiphysics.ConstitutiveLawsSmallStrainApplication as CLSSApp
    ...
    def ExecuteInitializeSolutionStep(self):
        ...
        if self.interval.IsInInterval(current_time):
            ...
            for element in self.model_part.Elements:
                if element.Has(CLSSApp.SERIAL_PARALLEL_IMPOSED_STRAIN): # With previous imposed strain
                    current_strain = element.GetValue(CLSSApp.SERIAL_PARALLEL_IMPOSED_STRAIN)
                    if step == 1:
                        element.SetValue(CLSSApp.SERIAL_PARALLEL_IMPOSED_STRAIN, current_strain * current_load_factor)
                    else:
                        element.SetValue(CLSSApp.SERIAL_PARALLEL_IMPOSED_STRAIN, current_strain * current_load_factor / self.old_load_factor)
