    ...
    )
{
    ...
    if( r_constitutive_law_options.Is(ConstitutiveLaw::COMPUTE_STRESS) ||
        ...
        trial_yield_function = this->YieldFunction(norm_trial_stress_dev, rValues, mEquivalentPlasticStrain, 0.0);
...
double SmallStrainJ2ThermalPlasticity3D::GetYieldStrength(ConstitutiveLaw::Parameters& rValues,
    const double accumulated_plastic_strain)
{
    ...
    double ys = mpPhaseTransSystem->GetYieldStrength(r_properties, r_geom, r_N, r_process_info);
...
)
{
    ...
    double yf_new = this->GetYieldStrength(rValues, AccumulatedPlasticStrainOld);
    ...
    while (yf_new > tolerance && num_iterations < 300)
    {
        ...
        const double dyf = -2. * mu * (1. + this->GetHardenRate(rValues, accumulated_plastic_strain) / (3. * mu));
        Dgamma -= yf / dyf;
    ...
    )
{
    ...
    return (NormTrialStressDev - Globals::SqrtTwoThird * this->GetYieldStrength(rValues, AccumulatedPlasticStrain) - 2. * mu * Dgamma);
}

//************************************************************************************
//************************************************************************************
double SmallStrainJ2ThermalPlasticity3D::GetHardenRate(
    ConstitutiveLaw::Parameters& rValues,
    const double AccumulatedPlasticStrain
    )
{
    cconst Properties& r_properties = rValues.GetMaterialProperties();
    const auto &r_geom = rValues.GetElementGeometry();
    const auto &r_N = rValues.GetShapeFunctionsValues();
    const auto &r_process_info = rValues.GetProcessInfo();
    ...
    double E = mpPhaseTransSystem->GetElasticityModulus(r_properties, r_geom, r_N, r_process_info);
    double NU = mpPhaseTransSystem->GetPoisson(r_properties, r_geom, r_N, r_process_info);
    double harden_rate = mpPhaseTransSystem->GetHardenRate(r_properties, r_geom, r_N, r_process_info);
...
void SmallStrainJ2ThermalPlasticity3D::save(Serializer& rSerializer) const
{
    ...
    rSerializer.save("PhaseTransSystem", mpPhaseTransSystem);
}
...
void SmallStrainJ2ThermalPlasticity3D::load(Serializer& rSerializer)
{
    ...
    rSerializer.load("PhaseTransSystem", mpPhaseTransSystem);
}
