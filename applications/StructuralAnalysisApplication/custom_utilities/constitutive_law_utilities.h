...
class KRATOS_API(STRUCTURAL_ANALYSIS_APPLICATION) ConstitutiveLawUtilities
{
  public:
    ///@name Type definitions
    ///@{
        ...
        )
    {
        if constexpr (Dimension == 3) {
            ...
            for (IndexType i = 0; i < Dimension; ++i)
                rDeviator[i] -= p_mean;
            rJ2 = 1/2 * (rDeviator[0] * rDeviator[0] + 
                         rDeviator[1] * rDeviator[1] + 
                         rDeviator[2] * rDeviator[2] + 
                         2 * rDeviator[3] * rDeviator[3] + 
                         2 * rDeviator[4] * rDeviator[4] + 
                         2 * rDeviator[5] * rDeviator[5]);
            // for (IndexType i = 0; i < Dimension; ++i)
            ...
            rJ2 = 0.5 * (std::pow(rDeviator[0], 2.0) + std::pow(rDeviator[1], 2.0)) + std::pow(rDeviator[2], 2.0);
        }
    }
    ...
    */
    static double CalculateShearModulus(ConstitutiveLaw::Parameters &rValues);
        ...
        /**
    * It calculates the properties of multi-phase materials
    * @param rValues: The CL parameters
    */
    static double CalculateMultiPhaseProperties(const Variable<double> &r_origin_variable, 
                                                ConstitutiveLaw::Parameters &rValues);
