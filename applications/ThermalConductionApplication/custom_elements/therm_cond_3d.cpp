...
ThermCond3D::ThermCond3D(IndexType NewId, GeometryType::Pointer pGeometry)
...
ThermCond3D::ThermCond3D(IndexType NewId, GeometryType::Pointer pGeometry, PropertiesType::Pointer pProperties)
    : Element(NewId, pGeometry, pProperties)
{
}

Element::Pointer ThermCond3D::Create(IndexType NewId, NodesArrayType const& ThisNodes, PropertiesType::Pointer pProperties) const
{
    return Kratos::make_intrusive<ThermCond3D>(NewId, GetGeometry().Create(ThisNodes), pProperties);
}

Element::Pointer ThermCond3D::Create(IndexType NewId, GeometryType::Pointer pGeom, PropertiesType::Pointer pProperties) const
{
    return Kratos::make_intrusive<ThermCond3D>(NewId, pGeom, pProperties);
}

ThermCond3D::~ThermCond3D()
{
}
...
void ThermCond3D::CalculateLocalSystem(MatrixType& rLeftHandSideMatrix, VectorType& rRightHandSideVector, const ProcessInfo& rCurrentProcessInfo)
{
    KRATOS_TRY
...
void ThermCond3D::CalculateRightHandSide(VectorType& rRightHandSideVector, const ProcessInfo& rCurrentProcessInfo)
{
...
void ThermCond3D::InitializeSolutionStep(const ProcessInfo& CurrentProcessInfo)
{
    KRATOS_TRY
...
void ThermCond3D::EquationIdVector(EquationIdVectorType& rResult, const ProcessInfo& CurrentProcessInfo) const 
{
...
void ThermCond3D::GetDofList(DofsVectorType& ElementalDofList, const ProcessInfo& CurrentProcessInfo) const 
{
    unsigned int number_of_nodes = GetGeometry().PointsNumber();
    ConvectionDiffusionSettings::Pointer my_settings = CurrentProcessInfo.GetValue(CONVECTION_DIFFUSION_SETTINGS);
    const Variable<double>& rUnknownVar = my_settings->GetUnknownVariable();
...
/*double ThermCond3D::ComputeSmagorinskyViscosity(const BoundedMatrix<double, 4, 3 > & DN_DX, const double& h, const double& C, const double nu )
{
