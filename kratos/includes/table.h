...
public:
    ...
    TResultType& GetNearestRow(TArgumentType const& X) 				//返回类型似乎错误, 应该为result_row_type 
    {
    ...
    // Get the nesrest value for the given argument
    TResultType const& GetNearestRow(TArgumentType const& X)  const 			//返回类型似乎错误, 应该为result_row_type 
    {
        std::size_t size = mData.size();
    ...
    void insert(TArgumentType const& X, result_row_type const& Y)
    {
        ...
        else if(X <= mData.back().first)						// 此处判断语句似乎错误 应该为 (X > mData.back().first)
            mData.push_back(RecordType(X,Y));
        else
            for(std::size_t i = 1 ; i < size ; i++)
    ...
    // faster than insert.
    template<class TArrayType>
    void PushBack(TArgumentType const& X, result_row_type const& Y)				// 模版类型<TArrayType>似乎没用到
    {
        mData.push_back(RecordType(X,Y));
    }
...
template<>
class Table<double, double>							// 模版类 特化定义 针对<double, double>进行优化
{
public:
    ///@name Type Definitions
    ///@{
    ...
    // Get the integration for the given argument using piecewise linear
    TResultType GetIntegration(TArgumentType const& X1, TArgumentType const& X2) const
    {
        std::size_t size = mData.size();
        TResultType sum = 0;
        TResultType a, b, c;
        TArgumentType min, max;
        std::size_t i;

        KRATOS_ERROR_IF(size == 0) << "Get value from empty table" << std::endl;

        if(size==1) // constant table. Returning the only value we have.
            return mData[0].second[0] * (X2 - X1);

        min = mData[0].first;
        max = mData[size - 1].first;

        if(X1 <= min ) {
            if(X2 <= min ) {

                c = mData[1].first - min;
                a = (mData[1].second[0] - mData[0].second[0]) / c;
	b = (mData[0].second[0] * mData[1].first - mData[1].second[0] * min) / c;

                sum = (X2 - X1) * (a * (X2 + X1) / 2 + b);
            } else {
	...
	b = min - X1;
                ...
                sum = b * (mData[0].second[0] - 0.5 * a * b);

                for(i = 1; i < size; i++)
                    if(X2 <= mData[i].first) {
                        a = (mData[i].second[0] - mData[i-1].second[0]) / (mData[i].first - mData[i-1].first);
                        b = X2 - mData[i-1].first;
                        sum += b * (mData[i-1].second[0] + 0.5 * a * b);
                        break;
                    }  else {
                        sum += 0.5 * (mData[i].second[0] + mData[i-1].second[0]) * (mData[i].first - mData[i-1].first);
                    }
            }
        } else {
            if (X1 > max) {
	...
	i = size - 2;
	c = max - mData[i].first;
                a = (mData[i + 1].second[0] - mData[i].second[0]) / c;
	b = (mData[i].second[0] * max - mData[i + 1].second[0] * mData[i].first) / c;
                ...
                for(i = 1; i < size; i++)
                    if (X1 <= mData[i].first) {
                        if (X2 <= mData[i].first) {
                            c = mData[i].first - mData[i - 1].first;
                            a = (mData[i] .second[0] - mData[i - 1].second[0]) / c;
                            b = (mData[i - 1].second[0] * mData[i].first - mData[i].second[0] * mData[i - 1].first) / c;
                            ...
                            sum = (mData[i].first - X1) * (a * (mData[i].first + X1) / 2 + b);
	        }
	        ...
	        b = X2 - mData[i-1].first;
                ...
                if (X2 > max) {
                    ...
                    sum += b * (mData[i].second[0] + 0.5 * a * b);
                }
            }
        }
