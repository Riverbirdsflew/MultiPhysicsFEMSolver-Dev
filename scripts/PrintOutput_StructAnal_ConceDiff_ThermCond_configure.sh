#!/bin/bash
exec > >(tee -a Struct-Concen-Diff_sh_output.txt)  # 重定向标准输出到 tee
exec 2> >(tee -a Struct-Concen-Diff_sh_output.txt >&2)  # 重定向标准错误到 tee
...
# Set mpi metis trilinos configuration
...
add_app ${KRATOS_APP_DIR}/ConstitutiveLawsSmallStrainApplication
add_app ${KRATOS_APP_DIR}/TrilinosApplication
add_app ${KRATOS_APP_DIR}/MetisApplication
add_app ${KRATOS_APP_DIR}/ConcentrationDiffusionApplication
add_app ${KRATOS_APP_DIR}/ThermalConductionApplication
add_app ${KRATOS_APP_DIR}/StructuralAnalysisApplication
