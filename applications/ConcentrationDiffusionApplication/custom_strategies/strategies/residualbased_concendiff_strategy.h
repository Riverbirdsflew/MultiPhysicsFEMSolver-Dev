...
#if !defined(KRATOS_RESIDUALBASED_CONCENTRATION_DIFFUSION_STRATEGY )
#define  KRATOS_RESIDUALBASED_CONCENTRATION_DIFFUSION_STRATEGY
...
//#include "custom_utilities/concentration_diffusion_settings.h"
...
/// This strategy is used to solve concentration-diffusion problem.
/**   Detail class definition.

The concentration-diffusion problem
...
template<class TSparseSpace,
         ...
         class TLinearSolver
         >
class ResidualBasedConcentrationDiffusionStrategy
...
public:
    ...
    KRATOS_CLASS_POINTER_DEFINITION( ResidualBasedConcentrationDiffusionStrategy );
    ...
    */
    ...
    @param model_part Reference to the ModelPart that contains the problem.
     @param pNewLinearSolver pointer to the solver for the concentration system.
    ...
    */
    ...
    ResidualBasedConcentrationDiffusionStrategy(
        ModelPart& model_part,
        typename TLinearSolver::Pointer pNewLinearSolver,
        bool ReformDofAtEachIteration = true,
        int time_order = 2,         //时间阶数
        int prediction_order = 2    //预测阶数
    )
        : ImplicitSolvingStrategy<TSparseSpace,TDenseSpace,TLinearSolver>(model_part,false)
    {
        KRATOS_TRY

        mtime_order = time_order;    //时间积分阶数 二阶
        mOldDt = 0.00;               //上一时间步长
        mprediction_order = prediction_order;   //预测步的阶数
...
//			mstep1->SetEchoLevel(2);
    ...
    /** Destructor.
    */
    virtual ~ResidualBasedConcentrationDiffusionStrategy() {}
    ...
    */
    ...
    double Solve() override
    {
      KRATOS_TRY
        ...
        //calculate the BDF coefficients//向后微分
        ProcessInfo& rCurrentProcessInfo = BaseType::GetModelPart().GetProcessInfo();
      double Dt = rCurrentProcessInfo[DELTA_TIME];
      int stationary= rCurrentProcessInfo[STATIONARY];

      double Dp_norm;//残差的范数，在什么地方计算
      if(stationary==1)
	{
	  CalculateProjection();
	  ...
	  if(mtime_order == 2)
	    {
	      ...
	      double rho = dt_old/Dt;  //两个时间步之间的比值
	      double coeff = 1.0/(Dt*rho*rho+Dt*rho); //对应的BDF系数

	      rCurrentProcessInfo[BDF_COEFFICIENTS].resize(3);
	      Vector& BDFcoeffs = rCurrentProcessInfo[BDF_COEFFICIENTS];
	      BDFcoeffs[0] =	coeff*(rho*rho+2.0*rho);	//coefficient for step n+1 //当前时间步的系数
	      BDFcoeffs[1] =	-coeff*(rho*rho+2.0*rho+1.0);//coefficient for step n  //前一个时间步的系数
	      BDFcoeffs[2] =	coeff; //对应前两个时间步的系数
	    }
	  else //如果是一阶的方法
	    {
    ...
    int Check() override
    {
        ...
        //~ if (BaseType::GetModelPart().NodesBegin()->SolutionStepsDataHas(rDensityVar) == false)
            //~ KRATOS_THROW_ERROR(std::logic_error, "Add  ----rDensityVar---- variable!!!!!! ERROR", "");//密度
       ...
       /* if(BaseType::GetModelPart().GetMesh().GetProperties(0)[EMISSIVITY] == false)
        ...
        if(BaseType::GetModelPart().GetMesh().GetProperties(0)[CONCEN_CONVECTION_COEFFICIENT] == false)
            KRATOS_THROW_ERROR(std::logic_error, "Add  ----CONCEN_CONVECTION_COEFFICIENT---- variable!!!!!! ERROR", "");
...
private:
    ...
    /** Copy constructor.
    */
    ResidualBasedConcentrationDiffusionStrategy(const ResidualBasedConcentrationDiffusionStrategy& Other);
...
#endif /* KRATOS_RESIDUALBASED_CONCENTRATION_DIFFUSION_STRATEGY  defined */
