    ...
    const ProcessInfo& rCurrentProcessInfo) const
{
    ...
    for (unsigned int i = 0; i < n_nodes; ++i){
        rResult[i] = r_geometry[i].GetDof(TEMPERATURE).EquationId();
    }
    ...
    const ProcessInfo& rCurrentProcessInfo) const
{
    ...
    for (unsigned int i = 0; i < n_nodes; ++i){
        rConditionalDofList[i] = r_geometry[i].pGetDof(TEMPERATURE);
    }
    ...
    ConditionDataStruct &rData)
{
    ...
    for (unsigned int i = 0; i < n_nodes; ++i) {
        rData.UnknownValues[i] = r_geometry[i].FastGetSolutionStepValue(TEMPERATURE);
        rData.FaceHeatFluxValues[i] = r_geometry[i].FastGetSolutionStepValue(FACE_HEAT_FLUX);
    }
    ...
    if(r_prop.HasTable(TIME, AMBIENT_TEMPERATURE)&&r_prop.HasTable(TIME, CONVECTION_COEFFICIENT)){
        Table table_AT = r_prop.GetTable(TIME, AMBIENT_TEMPERATURE);
        Table table_CC = r_prop.GetTable(TIME, CONVECTION_COEFFICIENT);
        double node_ambient_temperature = 0.0;
        double node_convection_coefficient = 0.0;
        double node_emissivity = 0.0;
        double time = rCurrentProcessInfo[TIME];
        for (unsigned int i = 0; i < n_nodes; ++i){
            node_ambient_temperature += table_AT.GetValue(time);
            node_convection_coefficient += table_CC.GetValue(time);
            std::cout<< "Node: "<<r_geometry[i].Id() << "Time: "<< time << std::endl;
        }
        rData.Emissivity = node_emissivity / n_nodes;
        rData.ConvectionCoefficient = node_convection_coefficient / n_nodes;
        rData.AmbientTemperature = node_ambient_temperature / n_nodes;
    }else{
    ...
    // rData.Emissivity = node_ambient_temperature / n_nodes;
    // rData.AmbientTemperature = node_ambient_temperature / n_nodes;
    // rData.ConvectionCoefficient = node_convection_coefficient / n_nodes;
