...
//         const double cr_nk = 0.5;
        ...
        double diffuse_conductivity = 0.0;
        //double specific_heat = 0.0;//比热
        //double density = 0.0;//密度

	//storing locally the flags to avoid repeated check in the nodal loops
        //const bool IsDefinedDensityVariable = my_settings->IsDefinedDensityVariable();//密度
        //const bool IsDefinedSpecificHeatVariableVariable = my_settings->IsDefinedSpecificHeatVariable();//比热
...
//             dphi_dt[i] = dt_inv*(phi[i] - phi_old [i];
        ...
        //noalias(rRightHandSideVector) = (dt_inv*density*specific_heat)*prod(aux1,phi_convected);//密度*比热=1
        noalias(rLeftHandSideMatrix)  = (dt_inv)*aux1;
        noalias(rRightHandSideVector) = (dt_inv)*prod(aux1,phi_convected);
        //adding the diffusion
        noalias(rLeftHandSideMatrix)  += (diffuse_conductivity *0.5 * prod(DN_DX, trans(DN_DX)))*static_cast<double>(TNumNodes);
        noalias(rRightHandSideVector) -= prod((diffuse_conductivity *0.5 * prod(DN_DX, trans(DN_DX))),phi_convected)*static_cast<double>(TNumNodes) ;
        ...
        const ProcessInfo& rCurrentProcessInfo) override
    {
        ...
        ConcentrationDiffusionSettings::Pointer p_my_settings = rCurrentProcessInfo.GetValue(CONCENTRATION_DIFFUSION_SETTINGS);
	    ...
	    // Storing locally the flags to avoid repeated check in the nodal loops
        //const bool is_defined_density_variable = p_my_settings->IsDefinedDensityVariable();//密度
        //const bool is_defined_specific_heat_variable = p_my_settings->IsDefinedSpecificHeatVariable();//比热
        ...
        for (unsigned int i = 0; i < TNumNodes; i++) {
			...
			if (is_defined_diffusion_variable) {
				...
				diffuse_conductivity += r_node.FastGetSolutionStepValue(r_diffusion_var);
			} // If not, the diffuse_conductivity is 0
        }
        ...
        // Mass matrix
        //const double aux_1 = dt_inv * density * specific_heat * volume / static_cast<double>(TNumNodes);//密度*比热=1
        const double aux_1 = dt_inv * volume / static_cast<double>(TNumNodes);
        ...
        const double aux_2 = diffuse_conductivity * 0.5 * volume;
    ...
    {
        return "LevelSetConcentrationElementSimplex #";
    }
