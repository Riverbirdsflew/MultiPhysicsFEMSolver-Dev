...
void TetrahedralMeshOrientationCheck::Execute()
{
    ...
    for (auto it_cond = mrModelPart.ConditionsBegin(); it_cond != mrModelPart.ConditionsEnd(); it_cond++) {
        ...
        // 如果是四边形或者四面体，边界是二维线或二维四边形
        else if(geometry_type == GeometryData::KratosGeometryType::Kratos_Quadrilateral3D4 || geometry_type == GeometryData::KratosGeometryType::Kratos_Line2D2 ){
            DenseVector<int> ids(r_geometry.size());
    ...
    for (auto it_elem = mrModelPart.ElementsBegin(); it_elem != mrModelPart.ElementsEnd(); it_elem++) {
        ...
        // 如果单元是四边形和六面体
        else if(geometry_type == GeometryData::KratosGeometryType::Kratos_Hexahedra3D8) {
            // Allocate a work array long enough to contain the Ids of a face
            auto faces = r_geometry.GenerateFaces();
            IndexType face_number = faces.size();
            IndexType face_size = faces[0].size();
            DenseVector<int> aux( face_size );

            // Loop over the faces
            for(IndexType fece_id=0; fece_id< face_number; fece_id++) {
                for(IndexType i=0; i<face_size; i++) {
                        aux[i] = faces[fece_id][i].Id();
                }
            ...
            auto edges = r_geometry.GenerateEdges();
            IndexType edge_number = edges.size();
            IndexType edge_size = edges[0].size();
            DenseVector<int> aux( edge_size );

            // Loop over the faces
            for(IndexType edge_id=0; edge_id< edge_number; edge_id++) {
                for(IndexType i=0; i<edge_size; i++) {
                        aux[i] = edges[edge_id][i].Id();
                }
