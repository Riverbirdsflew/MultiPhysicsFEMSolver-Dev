...
public:
    ...
    */
    ...
    //返回单元名称
    virtual std::string GetElementTypeAsString() const
    {
        std::string ElementName = "Element";
        
        const int node_number = (this->GetGeometry()).size();
        const int domain_size = this->GetGeometry().WorkingSpaceDimension();
        
        std::string dimension, nodes_number;
        nodes_number = std::to_string(node_number) + "N";
        if (domain_size == 2)
            dimension = "2D";
        else if (domain_size == 3)
            dimension = "3D";
        else
            KRATOS_ERROR << "No such Domain Size: " << dimension << std::endl;
        return ElementName + dimension + nodes_number; // 返回实际的单元类型名称，如果有不同的派生单元，可以根据具体情况返回不同的字符串。
    }
