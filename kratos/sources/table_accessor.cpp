...
{
    return GetValueFromTable(*mpInputVariable, rVariable, rProperties, rNode, rProcessInfo);
}

/***********************************************************************************/
/***********************************************************************************/

double TableAccessor::GetDerivative(
...
{
    // check the Derivative Variable Object
    if (rDerVariable == (*mpInputVariable))
        return GetDerivativeFromTable(*mpInputVariable, rVariable, rProperties, rGeometry, rShapeFunctionVector, rProcessInfo);
    else
        return 0.0;
}
    ...
    ) const
{
    // check the Derivative Variable Object
    if (rDerVariable == (*mpInputVariable))
        return GetDerivativeFromTable(*mpInputVariable, rVariable, rProperties, rNode, rProcessInfo);
    else
        return 0.0;
}
...
double TableAccessor::GetIntegration(
...
{
    // check the Integration Variable Object
    if (rIntVariable == (*mpInputVariable))
        return GetIntegrationFromTable(*mpInputVariable, rVariable, rProperties, rGeometry, rShapeFunctionVector, rProcessInfo);
    else
        return 0.0;
}
    ...
    ) const
{
    // check the Integration Variable Object
    if (rIntVariable == (*mpInputVariable))
        return GetIntegrationFromTable(*mpInputVariable, rVariable, rProperties, rNode, rProcessInfo);
    else
        return 0.0;
}
        ...
        const ProcessInfo& rProcessInfo) const
{
    ...
    if (mInputVariableType == Globals::DataLocation::NodeHistorical) {
        KRATOS_DEBUG_ERROR_IF_NOT(rNode.SolutionStepsDataHas(rIndependentVariable)) << "The Variable " << rIndependentVariable.Name() << " is not available at the nodes of the Geometry to retrieve Table values." << std::endl;
        nodal_value = rNode.FastGetSolutionStepValue(rIndependentVariable);
    } else if (mInputVariableType == Globals::DataLocation::NodeNonHistorical) {
        KRATOS_DEBUG_ERROR_IF_NOT(rNode.Has(rIndependentVariable)) << "The Variable " << rIndependentVariable.Name() << " is not available at the nodes of the Geometry to retrieve Table values." << std::endl;
        nodal_value = rNode.GetValue(rIndependentVariable);        
    } else {
    ...
    // Retrieve the dependent variable from the table
    const auto& r_table = rProperties.GetTable(rIndependentVariable, rDependentVariable);
    return r_table.GetValue(nodal_value);
}

/***********************************************************************************/
/***********************************************************************************/

double TableAccessor::GetDerivativeFromTable(
        const Variable<double> &rIndependentVariable,
        const Variable<double> &rDependentVariable,
        const Properties& rProperties,
        const GeometryType& rGeometry,
        ...
        const ProcessInfo& rProcessInfo) const
{
    ...
    // Retrieve the dependent variable from the table
    const auto& r_table = rProperties.GetTable(rIndependentVariable, rDependentVariable);
    return r_table.GetDerivative(nodal_value);
}

/***********************************************************************************/
/***********************************************************************************/

double TableAccessor::GetIntegrationFromTable(
        const Variable<double> &rIndependentVariable,
        const Variable<double> &rDependentVariable,
        const Properties& rProperties,
        const GeometryType& rGeometry,
        ...
        const ProcessInfo& rProcessInfo) const
{
    double init_independent_at_gauss = 0.0;
    double independent_at_gauss = 0.0;
    ...
    // Retrieve the dependent variable from the table
    const auto& r_table = rProperties.GetTable(rIndependentVariable, rDependentVariable);
    return r_table.GetIntegration(init_independent_at_gauss, independent_at_gauss);
}
        ...
        const ProcessInfo& rProcessInfo) const
{
    ...
    // Retrieve the dependent variable from the table
    const auto& r_table = rProperties.GetTable(rIndependentVariable, rDependentVariable);
    return r_table.GetIntegration(init_nodal_value, nodal_value);
}
