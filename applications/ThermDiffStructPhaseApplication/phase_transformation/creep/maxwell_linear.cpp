...
namespace Kratos
{
     ...
     * Copy Constructor.
     */
    MWLine::MWLine(const MWLine &rOther) 
    {
        unsigned int i;
     ...
     * Clone Constructor.
     */
    CreepModel::Pointer MWLine::Clone() const
    {
        return Kratos::make_shared<MWLine>(MWLine(*this));
    }
    ...
    int MWLine::Initialize(const Properties& rMaterialProperties, PhaseType rType)
    {
        bool chk;
        int iret = 0x0;
        ...
        if(this->mstat == Status::UnSet)
        {
            ...
            chk = this->mPara[0] != nullptr && this->mPara[1] != nullptr && this->mPara[2] != nullptr
                && this->mPara[3] != nullptr && this->mPara[4] != nullptr;
            if (!chk)
     ...
     */
    // creep parameters
    int MWLine::GetCreepParameters(const Properties &rMaterialProperties, const GeometryType &rElementGeometry,
     ...
     * creep的参数1.factor_a 2.factor_stress、3.factor_creep 4.factor_temperature  5.factor_time
     */
    double MWLine::GetEqCreepRate(const Properties &rMaterialProperties, const GeometryType &rElementGeometry,
                                  const Vector &rShapeFunctionsValues, const ProcessInfo &rCurrentProcessInfo,
                                  SizeType NodeId)
    {
        unsigned int i;
    ...
    double MWLine::GetEqCreepRateMean(const Properties &rMaterialProperties, const GeometryType &rElementGeometry,
                                      const Vector &rShapeFunctionsValues, const ProcessInfo &rCurrentProcessInfo,
                                      SizeType NodeId)
    {
        unsigned int i;
