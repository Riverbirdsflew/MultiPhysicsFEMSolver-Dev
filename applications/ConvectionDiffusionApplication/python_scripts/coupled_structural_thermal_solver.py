...
# Import applications
import KratosMultiphysics.StructuralAnalysisApplication as KratosSMA
        ...
        if self.domain_size == 2:
            ...
            for elem in self.thermal_solver.main_model_part.Elements:
                ...
                if elem.Info() == "Element2D3N":
                    replace_settings["element_name"].SetString("EulerianConvDiff2D3N")
                elif elem.Info() == "Element2D4N":
                    replace_settings["element_name"].SetString("EulerianConvDiff2D4N")
                
                # 执行替换
                KratosMultiphysics.ReplaceElementsAndConditionsProcess(self.thermal_solver.main_model_part, replace_settings).Execute()
            #modeler.DuplicateElements(self.structural_solver.main_model_part,
                                        #self.thermal_solver.main_model_part,
                                        #"EulerianThermCondElement2D3N")
            # ~ # 通过单元节点数来分别替换三节点和四节点单元
            # ~ for elem in self.thermal_solver.main_model_part.Elements:
                # ~ num_nodes = len(elem.GetNodes())
                # ~ if elem.Info() == "Element2D3N":
            ...
            for elem in self.structural_solver.main_model_part.Elements:
                struct_elem_name = "SmallDisplacementElement3D8N"
                break
            #print(struct_elem_name,struct_elem_name,struct_elem_name)
            if struct_elem_name == "Element3D4N" or struct_elem_name == "SmallDisplacementElement3D4N":
                modeler.GenerateModelPart(self.structural_solver.main_model_part,
                                      self.thermal_solver.main_model_part,
                                      "EulerianConvDiff3D4N",
                                      "ThermalFace3D3N")
            elif struct_elem_name == "Element3D8N" or struct_elem_name == "SmallDisplacementElement3D8N":
                modeler.GenerateModelPart(self.structural_solver.main_model_part,
                                      self.thermal_solver.main_model_part,
                                      "EulerianConvDiff3D8N",
                                      "ThermalFace3D4N")
            else:
                raise Exception("GenerateThermalModelPart error: Element type not supported")
            # for elem in self.thermal_solver.main_model_part.Elements:
            #     num_nodes = len(elem.GetNodes())
            #     if elem.Info() == "Element3D4N":
            #         replace_settings["element_name"].SetString("EulerianThermCondElement3D4N")
            #     elif elem.Info() == "Element3D8N":
            #         replace_settings["element_name"].SetString("EulerianThermCondElement3D8N")
