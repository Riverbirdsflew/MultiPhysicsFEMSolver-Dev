...
//为了混合单元定义的热模型生成器
void ConnectivityPreserveModeler::GenerateModelPartMixElemTherm(
    ModelPart& rOriginModelPart,
    ModelPart& rDestinationModelPart,
    Element const& rReferenceElement,
    Condition const& rReferenceBoundaryCondition)
{
    ...
    //std::cout <<"ElementType input:" << rReferenceElement.GetElementTypeAsString() << std::endl;
    this->DuplicateElementsTherm(rOriginModelPart, rDestinationModelPart, rReferenceElement);
...
//为了混合单元定义的扩散模型生成器
void ConnectivityPreserveModeler::GenerateModelPartMixElemDiff(
    ModelPart& rOriginModelPart,
    ModelPart& rDestinationModelPart,
    Element const& rReferenceElement,
    Condition const& rReferenceBoundaryCondition)
{
    ...
    //std::cout <<"ElementType input:" << rReferenceElement.GetElementTypeAsString() << std::endl;
    this->DuplicateElementsDiff(rOriginModelPart, rDestinationModelPart, rReferenceElement);
...
//根据问题类型（热问题），对于单元加前缀，然后复制
void ConnectivityPreserveModeler::DuplicateElementsTherm(
    ModelPart& rOriginModelPart,
    ModelPart& rDestinationModelPart,
    const Element& rReferenceElement) const
{
    ...
    for (auto i_elem = rOriginModelPart.ElementsBegin(); i_elem != rOriginModelPart.ElementsEnd(); ++i_elem) {
        ...
        // 获取原始单元名称
        std::string originalElementTypeName = i_elem->GetElementTypeAsString();//不知道有没有.Name() 待定
        //std::cout <<"ElementType input:" << originalElementTypeName;
        // 去除旧前缀（假设旧前缀是固定长度且已知的）
        std::string elementNameWithoutPrefix;
        if (originalElementTypeName.substr(0, strlen("SmallDisplacement")) == "SmallDisplacement") {
            elementNameWithoutPrefix = originalElementTypeName.substr(strlen("SmallDisplacement"));
        }
        else {
            elementNameWithoutPrefix = originalElementTypeName;
        }
        
        std::string ElementPrefix = rReferenceElement.GetElementTypeAsString();
        // 添加新前缀
        std::string newElementTypeName = "EulerianThermCond" + elementNameWithoutPrefix;
        
        //std::cout << newElementTypeName;
        
        // Reuse the geometry of the old element (to save memory)
        Element::Pointer p_element = KratosComponents<Element>::Get(newElementTypeName).Create(i_elem->Id(), i_elem->pGetGeometry(), properties);
...
//根据问题类型（扩散问题），对于单元加前缀，然后复制
void ConnectivityPreserveModeler::DuplicateElementsDiff(
    ModelPart& rOriginModelPart,
    ModelPart& rDestinationModelPart,
    const Element& rReferenceElement) const
{
    ...
    for (auto i_elem = rOriginModelPart.ElementsBegin(); i_elem != rOriginModelPart.ElementsEnd(); ++i_elem) {
        ...
        else if (originalElementTypeName.substr(0, strlen("Element")) == "Element") {
            elementNameWithoutPrefix = originalElementTypeName.substr(strlen("Element"));
        }
        else {
            elementNameWithoutPrefix = originalElementTypeName;
        }
        ...
        std::string ElementPrefix = rReferenceElement.GetElementTypeAsString();
        // 添加新前缀
        std::string newElementTypeName = "EulerianConcenDiff" + elementNameWithoutPrefix;
