...
void ModelPartIO::ReadModelPart(ModelPart & rThisModelPart)
{
    ...
    while(true) {
        ...
        } else if(word == "M4dTable") {
            if (mOptions.IsNot(IO::MESH_ONLY)) {
                ReadM4dTableBlock(rThisModelPart.M4dTables());
            } else {
                SkipBlock("M4dTable");
            }
...
//     WriteModelPartDataBlock(rThisModelPart); // TODO: FINISH ME
    ...
    if (mOptions.IsNot(IO::MESH_ONLY))
    {
        ...
        WriteM4dTableBlock(rThisModelPart.M4dTables());
    }
    ...
    const PartitioningInfo& rPartitioningInfo)
{
    ...
    while(true)
    {
        ...
        if(word == "ModelPartData")
            ...
            DivideM4dTableBlock(rOutputFiles);
        else if(word == "Properties")
...
/// read M4dTable block data
template<class M4dTablesContainerType>
void ModelPartIO::ReadM4dTableBlock(M4dTablesContainerType& rM4dTables)
{
    KRATOS_TRY

    ModelPart::M4dTableType temp_m4dtable;
    SizeType rdim, m, n, i, j;
    std::string word;
    std::string variable_name;
    std::array<VariableData* ,4> rp_x_variable;

    // read the dimension firstly
    ReadWord(word);
    ExtractValue(word, rdim);
    temp_m4dtable.Dim() = rdim;

    // read the x_variable_name
    if (rdim >= 1) {
        ReadWord(variable_name);

        if(!KratosComponents<VariableData>::Has(variable_name)) {
            std::stringstream buffer;
            buffer << variable_name << " is not a valid argument variable!!! M4dTable only accepts double arguments." << std::endl;
            buffer << " [Line " << mNumberOfLines << " ]";
            KRATOS_ERROR << buffer.str() << std::endl;
        }

        rp_x_variable[0] = KratosComponents<VariableData>::pGet(variable_name);
        temp_m4dtable.SetNameOfX(0, variable_name);
    } else {
        rp_x_variable[0] = KratosComponents<VariableData>::pGet(variable_name);
    }
    ...
    if (rdim >= 2) {
        ...
        rp_x_variable[1] = KratosComponents<VariableData>::pGet(variable_name);
        temp_m4dtable.SetNameOfX(0, variable_name);
    } else {
        rp_x_variable[1] = KratosComponents<VariableData>::pGet(variable_name);
    }
    ...
    if (rdim >= 3) {
        ...
        rp_x_variable[2] = KratosComponents<VariableData>::pGet(variable_name);
        temp_m4dtable.SetNameOfX(0, variable_name);
    } else {
        rp_x_variable[2] = KratosComponents<VariableData>::pGet(variable_name);
    }
    ...
    if (rdim >= 4) {
        ...
        rp_x_variable[3] = KratosComponents<VariableData>::pGet(variable_name);
        temp_m4dtable.SetNameOfX(0, variable_name);
    } else {
        rp_x_variable[3] = KratosComponents<VariableData>::pGet(variable_name);
    }

    // read the y_variable_name
    ReadWord(variable_name);

    if(!KratosComponents<VariableData>::Has(variable_name)) {
        std::stringstream buffer;
        buffer << variable_name << " is not a valid value variable!!! M4dTable only accepts double values." << std::endl;
        buffer << " [Line " << mNumberOfLines << " ]";
        KRATOS_ERROR << buffer.str() << std::endl;
    }

    VariableData const& r_y_variable = KratosComponents<VariableData>::Get(variable_name);
    temp_m4dtable.SetNameOfY(variable_name);

    // read data of x
    n = 1;

    for(i = 0; i < rdim && (!mpStream->eof()); i++) {
        double x;
        ReadWord(word);
        ExtractValue(word, m);
        n *= m;

        for(j = 0; j < m && (!mpStream->eof()); j++) {
            ReadWord(word);
            ExtractValue(word, x);	
            temp_m4dtable.PushBack(i, x);
        }
    }

    // read data of y
    for(i = 0; i < n && (!mpStream->eof()); i++) {
        double y;
        ReadWord(word);
        ExtractValue(word, y);
        temp_m4dtable.PushBack(y);
    }

    // sort the data of m4dtable
    temp_m4dtable.Sort();
    ...
    while(!mpStream->eof()) {
        ...
        if(CheckEndBlock("M4dTable", word))
            break;
    }

    if (rdim > 0 && rdim <= 4)
        rM4dTables.SetM4dTable(rdim, *rp_x_variable[0], *rp_x_variable[1], *rp_x_variable[2], *rp_x_variable[3], r_y_variable, temp_m4dtable);
    else
        KRATOS_ERROR << "M4dTable input dimension error!" << std::endl;

    KRATOS_CATCH("")
}

void ModelPartIO::ReadM4dTableBlock(ModelPart::M4dTablesContainerType& rM4dTables)
{
    KRATOS_TRY

    ModelPart::M4dTableType temp_m4dtable;
    SizeType m4dtable_id;
    SizeType rdim, m, n;
    std::string word;
    std::string variable_name;
    ...
    // read the x_variable_name
    for(SizeType i = 0; i < rdim; i++)
    {
        ReadWord(variable_name);
        temp_m4dtable.SetNameOfX(i, variable_name);
    }
    ...
    for(SizeType i = 0; i < rdim && (!mpStream->eof()); i++)
    {
        double x;
        ...
        for(SizeType j = 0; j < m && (!mpStream->eof()); j++)
        {
            ReadWord(word);
            ExtractValue(word, x);
    ...
    // read data of y
    for(SizeType i = 0; i < n && (!mpStream->eof()); i++)
    {
        double y;
    ...
    rM4dTables.insert(m4dtable_id, temp_m4dtable);

    KRATOS_CATCH("")
}

template<class M4dTablesContainerType>
void ModelPartIO::WriteM4dTableBlock(M4dTablesContainerType& rM4dTables)
{
    auto numM4dTables = rM4dTables.end() - rM4dTables.begin();
   
    for(unsigned int i = 0; i < numM4dTables; i++)
    {
        auto itM4dTable = rM4dTables.begin() + i;
        auto rdim = itM4dTable ->Dim();
        SizeType nn = 1;
        std::stringstream buffer;
        buffer << "Begin M4dTable" << i << std::endl;

        for(unsigned int j = 0; j < rdim; j++)
        {
            buffer <<  itM4dTable ->NameOfX(j) << "\t";
        }

        buffer <<  itM4dTable ->NameOfY() << std::endl;

        for(unsigned int j = 0; j < rdim; j++)
        {
            auto &itX = (itM4dTable ->Argument())[j].second;
            auto num = itM4dTable ->Size(j);
            nn *= num;
            buffer <<  num;
        ...
        buffer << std::endl;
        buffer << "End M4dTable";
        (*mpStream) << buffer.str() << std::endl;
    }
}

void ModelPartIO::WriteM4dTableBlock(ModelPart::M4dTablesContainerType& rM4dTables)
{
    auto numM4dTables = rM4dTables.end() - rM4dTables.begin();
...
void ModelPartIO::ReadPropertiesBlock(PropertiesContainerType& rThisProperties)
{
    ...
    while(!mpStream->eof())
    {
        ...
        if(variable_name == "Begin") // here we have some nested block.
        {
            ...
            if(variable_name == "Table") 	// At this moment the only supported nested block is a table or a m4dtable
                ReadTableBlock(temp_properties);
            
            if(variable_name == "M4dTable")
                ReadM4dTableBlock(temp_properties);
        }
        else if(KratosComponents<Variable<std::string> >::Has(variable_name))
        {
            std::string value;
...
void ModelPartIO::ReadSubModelPartBlock(ModelPart& rMainModelPart, ModelPart& rParentModelPart)
{
    ...
    while (true)
    {
        ...
        } else if (word == "SubModelPartM4dTables") {
            if (mOptions.IsNot(IO::MESH_ONLY)) {
                ReadSubModelPartM4dTablesBlock(rMainModelPart, r_sub_model_part);
            } else {
                SkipBlock("SubModelPartM4dTables");
            }
...
//                     }
        ...
        // Submodelpart m4dtables section, need to modify
        (*mpStream) << InitialTabulation  << "\tBegin SubModelPartM4dTables" << std::endl;
//                     ModelPart::TablesContainerType& rThisTables = rMainModelPart.Tables();
//                     auto numTables = rThisTables.end() - rThisTables.begin();
//                     for(unsigned int i = 0; i < numTables; i++)
//                     {
...
//                         (*mpStream) << InitialTabulation << "\t" << itTable->Id() << std::endl; //FIXME: Tables does not have Id() Whyyyyy?
//                     }
        (*mpStream) << InitialTabulation << "\tEnd SubModelPartM4dTables" << std::endl;
...
void  ModelPartIO::ReadSubModelPartM4dTablesBlock(ModelPart& rMainModelPart, ModelPart& rSubModelPart)
{
    KRATOS_TRY
    ...
    {
        ReadWord(word); // Reading the node id or End
        if (CheckEndBlock("SubModelPartM4dTables", word))
            break;

        ExtractValue(word, m4dtable_id);
        ModelPart::M4dTablesContainerType::iterator i_m4dtable = FindKey(rMainModelPart.M4dTables(), m4dtable_id, "M4dTable");
        rSubModelPart.AddM4dTable((i_m4dtable.base())->first, (i_m4dtable.base())->second);
    }
    KRATOS_CATCH("")
}
...
void ModelPartIO::DivideM4dTableBlock(OutputFilesContainerType& OutputFiles)
{
    KRATOS_TRY

    std::string block;

    WriteInAllFiles(OutputFiles, "Begin M4dTable ");

    ReadBlock(block, "M4dTable");
    WriteInAllFiles(OutputFiles, block);

    WriteInAllFiles(OutputFiles, "End M4dTable\n");
    ...
    PartitionIndicesContainerType const& ConditionsAllPartitions)
{
    ...
    while (!mpStream->eof())
    {
        ...
        if (word == "SubModelPartData")
            ...
            DivideSubModelPartM4dTableBlock(OutputFiles);
        else if (word == "SubModelPartNodes")
...
void ModelPartIO::DivideSubModelPartM4dTableBlock(OutputFilesContainerType& OutputFiles)
{
    KRATOS_TRY
    std::string block;

    WriteInAllFiles(OutputFiles, "Begin SubModelPartM4dTables");

    ReadBlock(block, "SubModelPartM4dTables");
    WriteInAllFiles(OutputFiles, block);

    WriteInAllFiles(OutputFiles, "End SubModelPartM4dTables\n");
    KRATOS_CATCH("")
}
