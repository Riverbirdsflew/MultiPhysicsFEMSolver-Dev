...
public:
    ...
    /**
     * @brief Retrieves a component pointer with the specified name.
     * @details This function retrieves a component pointer from the ComponentsContainer using the provided name.
     * @param rName The name of the component to retrieve.
     * @return A pointer to the retrieved component.
     * @note If the component is not found in debug, an error message will be printed and the program may terminate.
     */
    static const TComponentType* pGet(const std::string& rName)
    {
        auto it_comp =  msComponents.find(rName);
        KRATOS_DEBUG_ERROR_IF(it_comp == msComponents.end()) << GetMessageUnregisteredComponent(rName) << std::endl;
        return it_comp->second;
...
void KRATOS_API(KRATOS_CORE) AddKratosComponent(const std::string& rName, const Variable<Quaternion<double>>& rComponent);
