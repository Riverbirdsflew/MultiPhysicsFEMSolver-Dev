...
#include "axisymmetric_eulerian_thermal_conduction.h"

namespace Kratos
{

template< unsigned int TDim, unsigned int TNumNodes >
void AxisymmetricEulerianThermalConductionElement<TDim,TNumNodes>::CalculateLocalSystem(
    ...
    for (IndexType g = 0; g < n_gauss; ++g) {
        ...
        for (IndexType i = 0; i < TNumNodes; ++i) {
            for (IndexType j = 0; j < TNumNodes; ++j) {
                ...
                rRightHandSideVector(i) -= aux_dyn * (Variables.t_now[j] - Variables.t_old[j]);
                ...
                rRightHandSideVector(i) -= aux_diff * Variables.theta * Variables.t_now[j];
                rRightHandSideVector(i) -= aux_diff * (1.0 - Variables.theta) * Variables.t_old[j];
...
template< unsigned int TDim, unsigned int TNumNodes >
void AxisymmetricEulerianThermalConductionElement< TDim, TNumNodes >::CalculateRightHandSide(
    VectorType& rRightHandSideVector,
    const ProcessInfo& rCurrentProcessInfo)
{
    // Resize of RHS array
    ...
    for (IndexType g = 0; g < n_gauss; ++g) {
        ...
        for (IndexType i = 0; i < TNumNodes; ++i) {
            for (IndexType j = 0; j < TNumNodes; ++j) {
                ...
                rRightHandSideVector(i) += aux_diff_1 * Variables.theta * Variables.t_now[j];
                rRightHandSideVector(i) += aux_diff_1 * (1.0 - Variables.theta) * Variables.t_old[j];

                const double aux_diff_2 = w_g * Variables.conductivity * (DN_DX_g(i, 0) * DN_DX_g(j, 0) + DN_DX_g(i, 1) * DN_DX_g(j, 1));
                rRightHandSideVector(i) -= aux_diff_2 * Variables.theta * Variables.t_now[j];
                rRightHandSideVector(i) -= aux_diff_2 * (1.0 - Variables.theta) * Variables.t_old[j];
...
template< unsigned int TDim, unsigned int TNumNodes >
int AxisymmetricEulerianThermalConductionElement< TDim, TNumNodes >::Check(const ProcessInfo &rCurrentProcessInfo) const
{
    // Base element check
...
template class AxisymmetricEulerianThermalConductionElement<2,3>;
template class AxisymmetricEulerianThermalConductionElement<2,4>;
