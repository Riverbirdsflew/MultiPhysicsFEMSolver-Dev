    ...
    for (int i = 0; i < static_cast<int>(r_elements.size()); ++i) {
        ...
        if (r_properties.Has(YOUNG_MODULUS)) {
            for(IndexType i = 0; i < r_geometry.size(); ++i) {
                E += r_properties.GetValue(YOUNG_MODULUS, r_geometry[i], r_current_process_info);
            }
            E /= r_geometry.size();
        } else {
        ...
        if (r_properties.Has(POISSON_RATIO)) {
            for(IndexType i = 0; i < r_geometry.size(); ++i) {
                nu += r_properties.GetValue(POISSON_RATIO, r_geometry[i], r_current_process_info);
            }
            nu /= r_geometry.size();
        } else {
            nu = -1.0;
        ...
        if (r_properties.Has(DENSITY)) {
            for(IndexType i = 0; i < r_geometry.size(); ++i) {
                rho += r_properties.GetValue(DENSITY, r_geometry[i], r_current_process_info);
            }
            rho /= r_geometry.size();
        } else {
            check_has_all_variables = false;
