...
public:
    ...
    using TablesContainerType = std::unordered_map<std::size_t, TableType>; 		// Now, the key has been changed to hash value. Marx.

    using M4dTablesContainerType = std::unordered_map<std::size_t, M4dTableType>;
    ...
    // now it can be compatible with Table and M4dTable objects
    template<class TVariableType>
    typename TVariableType::Type GetValue(const TVariableType& rVariable, Node& rThisNode, const ProcessInfo& rProcessInfo)
    {
        auto it_value = mAccessors.find(rVariable.Key());

        if (it_value != mAccessors.end()) {
            return (it_value->second)->GetValue(rVariable, *this, rThisNode, rProcessInfo);
        } else if (mData.Has(rVariable)) {
            return mData.GetValue(rVariable);
        } else {
            return rThisNode.GetValue(rVariable);
        }        
    ...
    // now it can be compatible with Table and M4dTable objects
    template<class TVariableType>
    typename TVariableType::Type const GetValue(const TVariableType& rVariable, Node const& rThisNode, const ProcessInfo& rProcessInfo) const
    {
        auto it_value = mAccessors.find(rVariable.Key());
    ...
    // now it can be compatible with Table and M4dTable objects
    template<class TVariableType>
    typename TVariableType::Type GetValue(const TVariableType& rVariable, Node& rThisNode, IndexType SolutionStepIndex, const ProcessInfo& rProcessInfo)
    {
        auto it_value = mAccessors.find(rVariable.Key());
    ...
    // now it can be compatible with Table and M4dTable objects
    template<class TVariableType>
    typename TVariableType::Type const GetValue(const TVariableType& rVariable, Node const& rThisNode, IndexType SolutionStepIndex, const ProcessInfo& rProcessInfo) const
    {
        auto it_value = mAccessors.find(rVariable.Key());
    ...
    */
    ...
    /**
     * @brief append for m4dtable
     */
    template<class TXVariableType, class TYVariableType>
    M4dTableType& GetM4dTable(const IndexType rdim, const TXVariableType& X1Variable, const TXVariableType& X2Variable, const TXVariableType& X3Variable, const TXVariableType& X4Variable, const TYVariableType& YVariable)
    {
        return mM4dTables[M4dKey(rdim, X1Variable.Key(), X2Variable.Key(), X3Variable.Key(), X4Variable.Key(), YVariable.Key())];
    }

    template<class TXVariableType, class TYVariableType>
    M4dTableType const& GetM4dTable(const IndexType rdim, const TXVariableType& X1Variable, const TXVariableType& X2Variable, const TXVariableType& X3Variable, const TXVariableType& X4Variable, const TYVariableType& YVariable) const
    {
        return mM4dTables.at(M4dKey(rdim, X1Variable.Key(), X2Variable.Key(), X3Variable.Key(), X4Variable.Key(), YVariable.Key()));
    }

    template<class TXVariableType, class TYVariableType>
    void SetM4dTable(const IndexType rdim, const TXVariableType& X1Variable, const TXVariableType& X2Variable, const TXVariableType& X3Variable, const TXVariableType& X4Variable, const TYVariableType& YVariable, M4dTableType const& rThisM4dTable)
    {
        mM4dTables[M4dKey(rdim, X1Variable.Key(), X2Variable.Key(), X3Variable.Key(), X4Variable.Key(), YVariable.Key())] = rThisM4dTable;
    }

    bool HasM4dTables() const
    {
        return !mM4dTables.empty();
    }

    bool IsEmpty() const
    {
        return !( HasVariables() || HasTables() || HasM4dTables());
    }

    int64_t Key(std::size_t XKey, std::size_t YKey) const
    {
        int64_t result_key;
        std::string strXKey = std::to_string(XKey);
        std::string strYKey = std::to_string(YKey);

        result_key = FNV1a32Hash::CalculateHash(strXKey.c_str());
        result_key = result_key << 32;
        result_key |= FNV1a32Hash::CalculateHash(strYKey.c_str());
        return result_key;
    }

    int64_t M4dKey(const IndexType rdim, std::size_t X1Key, std::size_t X2Key, std::size_t X3Key, std::size_t X4Key, std::size_t YKey) const
    {
        int64_t result_key = 0;
        std::string strXKey;
        std::string strYKey = std::to_string(YKey);

        // chcek dimension and set strXKey
        if (rdim == 1) {
            strXKey = std::to_string(X1Key);
        } else if (rdim == 2) {
            strXKey = std::to_string(X1Key) + std::to_string(X2Key);
        } else if (rdim == 3) {
            strXKey = std::to_string(X1Key) + std::to_string(X2Key) + std::to_string(X3Key);
        } else if (rdim >= 4) {
            strXKey = std::to_string(X1Key) + std::to_string(X2Key) + std::to_string(X3Key) + std::to_string(X4Key);
        } else {
            return result_key;
        }  
    ...
    /**
     * @brief This method returns the m4dtables
     * @return The whole lis of m4dtables
     */
    M4dTablesContainerType& M4dTables()
    {
        return mM4dTables;
    }

    /**
     * @brief This method returns the m4dtables (constant)
     * @return The whole lis of m4dtables
     */
    M4dTablesContainerType const& M4dTables() const
    {
        return mM4dTables;
    }
    ...
    template<class TXVariableType, class TYVariableType>
    bool HasM4dTable(const IndexType rdim, const TXVariableType& X1Variable, const TXVariableType& X2Variable, const TXVariableType& X3Variable, const TXVariableType& X4Variable, const TYVariableType& YVariable) const
    {
        return (mM4dTables.find(M4dKey(rdim, X1Variable.Key(), X2Variable.Key(), X3Variable.Key(), X4Variable.Key(), YVariable.Key())) != mM4dTables.end());
    }
    ...
    void PrintData(std::ostream& rOStream) const override
    {
        ...
        // M4dTables
        if (mM4dTables.size() > 0) {
            // Print the tables
            rOStream << "This properties contains " << mM4dTables.size() << " m4dtables\n";
            for (auto& r_m4dtable : mM4dTables) {
                rOStream << "M4dTable key: " << r_m4dtable.first << "\n";
                StringUtilities::PrintDataWithIdentation(rOStream, r_m4dtable.second);
            }
        }
...
private:
    ...
    M4dTablesContainerType mM4dTables;      /// The m4dtables contained on the properties
    ...
    void save(Serializer& rSerializer) const override
    {
        ...
        rSerializer.save("M4dTables", mM4dTables);
    ...
    void load(Serializer& rSerializer) override
    {
        ...
        rSerializer.load("M4dTables", mM4dTables);
