    ...
    )
{
    ...
    const double E = ConstitutiveLawUtilities<TVoigtSize>::CalculateMultiPhaseProperties(YOUNG_MODULUS, rValues);
    const double nu = ConstitutiveLawUtilities<TVoigtSize>::CalculateMultiPhaseProperties(POISSON_RATIO, rValues);

    return E / (2.0 * (1.0 + nu));
}

template<SizeType TVoigtSize>
double ConstitutiveLawUtilities<TVoigtSize>::CalculateMultiPhaseProperties(
    ...
    )
{
    ...
    bool IsDefinedMuiltiPhase = false;
    if(r_material_properties.Has(IS_DEFINED_MULTIPHASE))
        IsDefinedMuiltiPhase = r_material_properties[IS_DEFINED_MULTIPHASE];

    double r_variable =0.0;
    const std::string r_variable_name = r_origin_variable.Name();
    
    if(IsDefinedMuiltiPhase){
        //得到变量名
        const std::string r_variable_name_m1 = r_variable_name + "_M1";
        const std::string r_variable_name_m2 = r_variable_name + "_M2";
        const std::string r_variable_name_m3 = r_variable_name + "_M3";
        const std::string r_variable_name_m4 = r_variable_name + "_M4";
        const std::string r_variable_name_m5 = r_variable_name + "_M5";
        const std::string r_variable_name_m6 = r_variable_name + "_M6";
        const std::string r_variable_name_m7 = r_variable_name + "_M7";
        
        //通过变量名获得变量 
        const double r_variable_m1 = r_material_properties.GetValue(KratosComponents<Variable<double>>::Get(r_variable_name_m1), r_geom, r_N, r_process_info);//通过accessor
        const double r_variable_m2 = r_material_properties.GetValue(KratosComponents<Variable<double>>::Get(r_variable_name_m2), r_geom, r_N, r_process_info);//通过accessor
        const double r_variable_m3 = r_material_properties.GetValue(KratosComponents<Variable<double>>::Get(r_variable_name_m3), r_geom, r_N, r_process_info);//通过accessor
        const double r_variable_m4 = r_material_properties.GetValue(KratosComponents<Variable<double>>::Get(r_variable_name_m4), r_geom, r_N, r_process_info);//通过accessor
        const double r_variable_m5 = r_material_properties.GetValue(KratosComponents<Variable<double>>::Get(r_variable_name_m5), r_geom, r_N, r_process_info);//通过accessor
        const double r_variable_m6 = r_material_properties.GetValue(KratosComponents<Variable<double>>::Get(r_variable_name_m6), r_geom, r_N, r_process_info);//通过accessor
        const double r_variable_m7 = r_material_properties.GetValue(KratosComponents<Variable<double>>::Get(r_variable_name_m7), r_geom, r_N, r_process_info);//通过accessor
        
        //std::cout << r_variable_name << "(multi phase):" << r_variable_m1 << ' ' << r_variable_m2 << ' ' << r_variable_m3 << ' ' << r_variable_m4 << ' ' << r_variable_m5 << ' ' << r_variable_m6 << ' ' << r_variable_m7 << std::endl;//调试代码

        double mass_fraction_m1_at_gauss = 0.0;
        double mass_fraction_m2_at_gauss = 0.0;
        double mass_fraction_m3_at_gauss = 0.0;
        double mass_fraction_m4_at_gauss = 0.0;
        double mass_fraction_m5_at_gauss = 0.0;
        double mass_fraction_m6_at_gauss = 0.0;
        double mass_fraction_m7_at_gauss = 0.0;
        //geom 获得node
        for (SizeType i = 0; i < r_N.size(); ++i){
            const double nodal_mass_fraction_m1 = r_geom[i].FastGetSolutionStepValue(MASS_FRACTION_M1);
            const double nodal_mass_fraction_m2 = r_geom[i].FastGetSolutionStepValue(MASS_FRACTION_M2);
            const double nodal_mass_fraction_m3 = r_geom[i].FastGetSolutionStepValue(MASS_FRACTION_M3);
            const double nodal_mass_fraction_m4 = r_geom[i].FastGetSolutionStepValue(MASS_FRACTION_M4);
            const double nodal_mass_fraction_m5 = r_geom[i].FastGetSolutionStepValue(MASS_FRACTION_M5);
            const double nodal_mass_fraction_m6 = r_geom[i].FastGetSolutionStepValue(MASS_FRACTION_M6);
            const double nodal_mass_fraction_m7 = r_geom[i].FastGetSolutionStepValue(MASS_FRACTION_M7);
            
            mass_fraction_m1_at_gauss += nodal_mass_fraction_m1 * r_N[i];
            mass_fraction_m2_at_gauss += nodal_mass_fraction_m2 * r_N[i];
            mass_fraction_m3_at_gauss += nodal_mass_fraction_m3 * r_N[i];
            mass_fraction_m4_at_gauss += nodal_mass_fraction_m4 * r_N[i];
            mass_fraction_m5_at_gauss += nodal_mass_fraction_m5 * r_N[i];
            mass_fraction_m6_at_gauss += nodal_mass_fraction_m6 * r_N[i];
            mass_fraction_m7_at_gauss += nodal_mass_fraction_m7 * r_N[i];
            //std::cout << "mass fraction m6: " << mass_fraction_m6_at_gauss <<std::endl;//调试代码
        }
        
        r_variable = r_variable_m1 * mass_fraction_m1_at_gauss + 
                            r_variable_m2 * mass_fraction_m2_at_gauss +
                            r_variable_m3 * mass_fraction_m3_at_gauss +
                            r_variable_m4 * mass_fraction_m4_at_gauss +
                            r_variable_m5 * mass_fraction_m5_at_gauss +
                            r_variable_m6 * mass_fraction_m6_at_gauss +
                            r_variable_m7 * mass_fraction_m7_at_gauss ;
    }
    else{
        r_variable = r_material_properties.GetValue(r_origin_variable, r_geom, r_N, r_process_info);
    }
    if(r_variable == 0.0) {
        KRATOS_ERROR << "Get multi phase properties failed! " << r_variable_name << ':' << r_variable << std::endl;
        //std::cout << "Get multi phase properties failed, get from constant properties! " << r_variable_name << ':' << r_variable << std::endl;//调试代码
    }
    return r_variable;
}
