...
public:
    ...
    typedef TablesContainerType::const_iterator TableConstantIterator;
     ...
     /// Defining a m4dtable with double argument and result type as m4dtable type.
    typedef M4dTable M4dTableType;

    /// The container of the m4dtables. A vector map of the m4dtables.
    typedef PointerVectorMap<SizeType, M4dTableType> M4dTablesContainerType;

    /** Iterator over the M4dTables. This iterator is an indirect
    iterator over M4dTables::Pointer which turn back a reference to
    M4dTable by * operator and not a pointer for more convenient
    usage. */
    typedef M4dTablesContainerType::iterator M4dTableIterator;

    /** Const iterator over the M4dTables. This iterator is an indirect
    iterator over M4dTables::Pointer which turn back a reference to
    M4dTable by * operator and not a pointer for more convenient
    usage. */
    typedef M4dTablesContainerType::const_iterator M4dTableConstantIterator;
    ...
    */
    ...
    SizeType NumberOfM4dTables() const
    {
        return mM4dTables.size();
    }

    /** Inserts a M4dTable
     */
    void AddM4dTable(IndexType M4dTableId, M4dTableType::Pointer pNewM4dTable);

    /** Returns the M4dTable::Pointer  corresponding to it's identifier */
    M4dTableType::Pointer pGetM4dTable(IndexType M4dTableId)
    {
        return mM4dTables(M4dTableId);
    }

    /** Returns a reference to M4dTable corresponding to the identifier */
    M4dTableType& GetM4dTable(IndexType M4dTableId)
    {
        return mM4dTables[M4dTableId];
    }

    /** Remove the M4dTable with given Id from current mesh in this modelpart and all its subs.
    */
    void RemoveM4dTable(IndexType M4dTableId);

    /** Remove the M4dTable with given Id from current mesh in parents, itself and all children.
    */
    void RemoveM4dTableFromAllLevels(IndexType M4dTableId);


    M4dTableIterator M4dTablesBegin()
    {
        return mM4dTables.begin();
    }

    M4dTableConstantIterator M4dTablesBegin() const
    {
        return mM4dTables.begin();
    }

    M4dTableIterator M4dTablesEnd()
    {
        return mM4dTables.end();
    }

    M4dTableConstantIterator M4dTablesEnd() const
    {
        return mM4dTables.end();
    }
    ...
    M4dTablesContainerType::ContainerType& M4dTablesArray()
    {
        return mM4dTables.GetContainer();
    }
...
private:
    ...
    M4dTablesContainerType mM4dTables; /// The m4dtables contained on the model part
