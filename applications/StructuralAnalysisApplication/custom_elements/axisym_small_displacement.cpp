...
#include "custom_utilities/structural_analysis_math_utilities.hpp"
    ...
    ) const
{
    ...
    const double Radius = StructuralAnalysisMathUtilities::CalculateRadius(N, GetGeometry());

    rB.clear();
    // 调整rB的尺寸
    rB.resize( 4, 2 * NumberOfNodes, false );
    noalias( rB ) = ZeroMatrix( 4, 2 * NumberOfNodes );
